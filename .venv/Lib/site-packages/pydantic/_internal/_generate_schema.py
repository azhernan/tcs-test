...
def modify_model_json_schema(
    ...
    *,
    ...
    docstring = (
        None
        if cls is BaseModel or is_builtin_dataclass(cls) or is_pydantic_dataclass(cls)
        else cls.__doc__
    )
...
class GenerateSchema:
    ...
    def _model_schema(self, cls: type[BaseModel]) -> core_schema.CoreSchema:
        ...
        with self.defs.get_schema_or_ref(cls) as (model_ref, maybe_schema):
            ...
            with self._config_wrapper_stack.push(config_wrapper), self._types_namespace_stack.push(
                cls
            ):
    ...
    def _typed_dict_schema(self, typed_dict_cls: Any, origin: Any) -> core_schema.CoreSchema:
        ...
        with self.model_type_stack.push(typed_dict_cls), self.defs.get_schema_or_ref(
            ...
            with self._config_wrapper_stack.push(config), self._types_namespace_stack.push(
                ...
                metadata = build_metadata_dict(
                    js_functions=[
                        partial(modify_model_json_schema, cls=typed_dict_cls, title=title)
                    ],
                )
    ...
    def _callable_schema(self, function: Callable[..., Any]) -> core_schema.CallSchema:
        ...
        mode_lookup: dict[
            _ParameterKind, Literal["positional_only", "positional_or_keyword", "keyword_only"]
        ] = {
        ...
        if isinstance(metadata, FieldInfo):
            ...
            if json_schema_update or json_schema_extra:
                CoreMetadataHandler(schema).metadata.setdefault(
                    "pydantic_js_annotation_functions", []
                ).append(get_json_schema_update_func(json_schema_update, json_schema_extra))
        return schema
...
_VALIDATOR_F_MATCH: Mapping[
    ...
    ("before", "no-info"): lambda f, schema, _: core_schema.no_info_before_validator_function(
        f, schema
    ),
    ("after", "no-info"): lambda f, schema, _: core_schema.no_info_after_validator_function(
        f, schema
    ),
    ("plain", "no-info"): lambda f, _1, _2: core_schema.no_info_plain_validator_function(f),
    ("wrap", "no-info"): lambda f, schema, _: core_schema.no_info_wrap_validator_function(
        f, schema
    ),
    (
        "before",
