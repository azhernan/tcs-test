...
def _get_immediate_cls_attr(cls: Type[Any], attrname: str, strict: bool = False) -> Optional[Any]:
    ...
    for base in cls.__mro__[1:]:
        ...
        if attrname in base.__dict__ and (
            base is cls
            or ((base in cls.__bases__ if strict else True) and not _is_classical_inherits)
        ):
...
class _ClassScanMapperConfig(_MapperConfig):
    ...
    def __init__(
        ...
        self.allow_unmapped_annotations = getattr(self.cls, "__allow_unmapped__", False) or bool(
                ...
                if name in _include_dunders:
                    ...
                    if _is_declarative_props(obj) and not obj._quiet:
                        util.warn(
                            ...
                            "not applying to subclass %s." % (base.__name__, name, base, cls)
                        )
                    ...
                    if isinstance(obj, (Column, MappedColumn)):
                        ...
                        if obj._cascading:
                            ...
                            collected_attributes[name] = column_copies[obj] = ret = obj.__get__(
                                obj, cls
                            )
                            ...
                            if (
                                ...
                                and not isinstance(ret.original_property, MapperProperty)
                            ):
                                ret = ret.descriptor
                        ...
                        if not fixed_table:
                            assert name in collected_attributes or attribute_is_overridden(
                                name, None
                            )
                        continue
                    else:
                    ...
                    generated_obj = (
                        collected_annotation.attr_value if collected_annotation is not None else obj
                    )
                    if obj is None and not fixed_table and is_mapped:
                        collected_attributes[name] = (
                            generated_obj if generated_obj is not None else MappedColumn()
                        )
        ...
        field_list = [
            ...
            for key, anno, mapped_container in (
                ...
                and (
                    ...
                    or not isinstance(self.collected_attributes[key], QueryableAttribute)
                )
            )
        ]
    ...
    def _collect_annotation(
        ...
        extracted = _extract_mapped_subtype(
            ...
            expect_mapped=expect_mapped and not is_dataclass,  # self.allow_dataclass_fields,
        )
    ...
    def _extract_mappable_attributes(self) -> None:
        ...
        for k in list(collected_attributes):
            ...
            if _is_declarative_props(value):
                ...
                isinstance(value, QueryableAttribute) and value.class_ is not cls and value.key != k
            ):
            ...
            # to change that now (2.0 betas)
                ...
                _undefer_column_name(k, self.column_copies.get(value, value))  # type: ignore
            else:
            ...
            if declared_columns:
                for c in declared_columns:
                    if not table.c.contains_column(c):
                        raise exc.ArgumentError(
                            "Can't add additional column %r when " "specifying __table__" % c.key
                        )
    ...
    def _prepare_mapper_arguments(self, mapper_kw: _MapperKwArgs) -> None:
        ...
        if "primary_key" in mapper_args:
            mapper_args["primary_key"] = [
                self.column_copies.get(v, v) for v in util.to_list(mapper_args["primary_key"])
            ]
        ...
        if self.inherits and not mapper_args.get("concrete", False):
            ...
            if "exclude_properties" not in mapper_args:
                mapper_args["exclude_properties"] = exclude_properties = {
                    c.key for c in inherited_table.c if c not in inherited_mapper._columntoproperty
    ...
    def map(self, mapper_kw: _MapperKwArgs = util.EMPTY_DICT) -> Mapper[Any]:
        ...
        if hasattr(self.cls, "__mapper_cls__"):
            mapper_cls = cast(
                "Type[Mapper[Any]]",
                util.unbound_method_to_callable(self.cls.__mapper_cls__),  # type: ignore
            )
        else:
            mapper_cls = Mapper
