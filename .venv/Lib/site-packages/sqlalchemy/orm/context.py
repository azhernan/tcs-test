...
class ORMSelectCompileState(ORMCompileState, SelectState):
    ...
    def _setup_for_generate(self):
        ...
        self.group_by = (
            self._adapt_col_list(util.flatten_iterator(query._group_by_clauses), current_adapter)
    ...
    def _compound_eager_statement(self):
        ...
        if self.order_by:
            ...
            unwrapped_order_by = [
                (elem.element if isinstance(elem, sql.elements._label_reference) else elem)
        ...
        inner = self._select_statement(
            self.primary_columns + [c for c in order_by_col_expr if c not in self.dedupe_columns],
            self.from_clauses,
        ...
        statement = future.select(*([inner] + self.secondary_columns))  # use_labels=self.labels
    ...
    def _get_current_adapter(self):
        ...
        def _adapt_clause(clause, as_filter):
            ...
            def replace(elem):
                is_orm_adapt = (
                    "_orm_adapt" in elem._annotations or "parententity" in elem._annotations
                )
    ...
    def _get_extra_criteria(self, ext_info):
        if (
            ...
            return tuple(
                ...
                for ae in self.global_attributes[("additional_entity_criteria", ext_info.mapper)]
                if (ae.include_aliases or ae.entity is ext_info) and ae._should_include(self)
            )
        else:
            return ()
        ...
        for fromclause in self.from_clauses:
            ...
            if (
                ...
                and (
                    ...
                    or ("additional_entity_criteria", ext_info.mapper) in self.global_attributes
                )
...
class _MapperEntity(_QueryEntity):
    ...
    def setup_compile_state(self, compile_state):
        ...
        if (
            ...
            or ("additional_entity_criteria", self.mapper) in compile_state.global_attributes
        ):
...
class _BundleEntity(_QueryEntity):
    ...
    def row_processor(self, context, result):
        procs, labels, extra = zip(*[ent.row_processor(context, result) for ent in self._entities])
