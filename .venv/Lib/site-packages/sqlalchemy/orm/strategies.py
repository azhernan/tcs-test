...
class AbstractRelationshipLoader(LoaderStrategy):
    ...
    def _immediateload_create_row_processor(
        ...
        return self.parent_property._get_strategy((("lazy", "immediate"),)).create_row_processor(
            context,
            query_entity,
            path,
            loadopt,
...
class LazyLoader(AbstractRelationshipLoader, util.MemoizedSlots, log.Identified):
    ...
    def __init__(self, parent: RelationshipProperty[Any], strategy_key: Tuple[Any, ...]):
        super().__init__(parent, strategy_key)
        ...
        if self.use_get:
            ...
            self.logger.info("%s will use Session.get() to optimize instance loads", self)

    def init_class_attribute(self, mapper):
        self.is_class_level = True

        _legacy_inactive_history_style = self.parent_property._legacy_inactive_history_style

        if self.parent_property.active_history:
            active_history = True
            _deferred_history = False

        elif self.parent_property.direction is not interfaces.MANYTOONE or not self.use_get:
    ...
    def _invoke_raise_load(self, state, passive, lazy):
        raise sa_exc.InvalidRequestError("'%s' is not available due to lazy='%s'" % (self, lazy))
        ...
        if use_get:
            ...
            if instance is not None:
                if instance is LoaderCallableStatus.PASSIVE_CLASS_MISMATCH:
                    ...
                    return instance
            elif not passive & PassiveFlag.SQL_OK or not passive & PassiveFlag.RELATED_OBJECT_OK:
                return LoaderCallableStatus.PASSIVE_NO_RESULT
            ...
            if l:
                if l > 1:
                    util.warn(
                        ...
                        "uselist=False for lazily-loaded attribute '%s' " % self.parent_property
                    )
...
class SubqueryLoader(PostLoader):
    ...
    def _generate_from_original_query(
        ...
        if not q._from_obj:
            ...
            q.select_from.non_generative(
                ...
                *{
                    ent["entity"]
                    for ent in _column_descriptions(orig_query, compile_state=orig_compile_state)
        ...
        target_cols = orig_compile_state._adapt_col_list(
            [sql.coercions.expect(sql.roles.ColumnsClauseRole, o) for o in leftmost_attr],
    ...
    def _prep_for_joins(self, left_alias, subq_path):
        ...
        local_attr = [
            getattr(parent_alias, self.parent._columntoproperty[c].key) for c in local_cols
        ]
        ...
        if orig_query._is_lambda_element:
            if context.load_options._lazy_loaded_from is None:
                util.warn(
                    ...
                    "selectinload() for more effective full-lambda caching" % (self, orig_query)
                )
...
class JoinedLoader(AbstractRelationshipLoader):
    ...
    def setup_query(
        ...
        **kwargs,
        ...
        if not compile_state.compile_options._enable_eagerloads:
            return
        elif loadopt and compile_state.statement is not None and compile_state.statement.is_dml:
            util.warn_deprecated(
        ...
        user_defined_adapter = (
            self._init_user_defined_eager_proc(loadopt, compile_state, compile_state.attributes)
            if loadopt
            else False
        )
    ...
    def _create_eager_join(
        ...
        should_nest_selectable = (
            compile_state.multi_row_eager_loaders and compile_state._should_nest_selectable
        )
        ...
        if adapter:
            if getattr(adapter, "is_aliased_class", False):
                ...
                onclause = getattr(
                    orm_util.AliasedClass(self.parent, adapter.selectable, use_mapper_path=True),
                    self.key,
                    self.parent_property,
                )
    ...
    def _splice_nested_inner_join(
        ...
        if isinstance(join_obj, sql.selectable.FromGrouping):
            ...
            # most call
            if entity_inside_join_structure is False:
                assert False, "assertion failed attempting to produce joined eager loads"
            return None
    ...
    def _create_eager_adapter(self, context, result, adapter, path, loadopt):
        ...
        user_defined_adapter = (
            self._init_user_defined_eager_proc(loadopt, compile_state, context.attributes)
            if loadopt
            else False
        )
    ...
    def create_row_processor(
        ...
        if eager_adapter is not False:
            ...
            self.parent_property._get_strategy((("lazy", "select"),)).create_row_processor(
                context,
                query_entity,
                path,
                loadopt,
...
class SelectInLoader(PostLoader, util.MemoizedSlots):
    ...
    def create_row_processor(
        ...
        (
            ...
            execution_options,
            recursion_depth,
        ) = self._setup_for_recursion(context, path, loadopt, join_depth=self.join_depth)
    ...
    def _load_for_path(
        ...
        if not query_info.load_only_child:
            our_states = [
                (state.key[1], state, state.dict, overwrite) for state, overwrite in states
            ]
        ...
        if not query_info.load_with_join:
            ...
            q = q.select_from(self._parent_alias).join(
                getattr(self._parent_alias, self.parent_property.key).of_type(effective_entity)
            )
        ...
        while our_keys:
            ...
            data = {
                ...
                for k, v in context.session.execute(
                    q,
                    params={
                        "primary_keys": [key[0] if query_info.zero_idx else key for key in chunk]
                    },
                    execution_options=execution_options,
                ).unique()
            }
        ...
        if value is not None and initiator.key == prop.key:
            ...
            if hasparent and oldvalue is not value:
                raise sa_exc.InvalidRequestError(
                    ...
                    "single parent." % (orm_util.instance_str(value), state.class_, prop),
                    code="bbf1",
                )
        return value
