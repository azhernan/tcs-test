    ...
    def __new__(cls, value_list: Optional[Union[Iterable[str], str]]) -> CascadeOptions:
        ...
        if values.difference(cls._allowed_cascades):
            raise sa_exc.ArgumentError(
                ...
                % ", ".join([repr(x) for x in sorted(values.difference(cls._allowed_cascades))])
            )
            ...
            result.append(
                sql.select(
                    *(
                        ...
                        + [sql.literal_column(sql_util._quote_ddl_expr(type_)).label(typecolname)]
                    )
                ).select_from(table)
            )
        else:
            result.append(sql.select(*[col(name, table) for name in colnames]).select_from(table))
    ...
    def __init__(
        ...
        self._adapter = ORMAdapter(
            ...
            adapt_from_selectables={
                m.selectable for m in self.with_polymorphic_mappers if not adapt_on_names
            },
...
class _WrapUserEntity:
    ...
    def __getattribute__(self, name):
        ...
        if name in subject.__dict__ and isinstance(subject.__dict__[name], decl_api.declared_attr):
    ...
    def __init__(
        ...
        if callable(where_criteria):
            ...
            self.where_criteria = lambdas.DeferredLambdaElement(
                ...
                opts=lambdas.LambdaOptions(track_closure_variables=track_closure_variables),
            )
        else:
...
def with_parent(
    ...
    if isinstance(prop, str):
        ...
        if mapper_property is None or not prop_is_relationship(mapper_property):
            raise sa_exc.ArgumentError(
                f"Expected relationship property for with_parent(), " f"got {mapper_property}"
            )
...
def _getitem(iterable_query: Query[Any], item: Any) -> Any:
    ...
    def _no_negative_indexes():
        raise IndexError("negative indexes are not accepted by SQL " "index / slice operators")
    ...
    if isinstance(item, slice):
        ...
        elif (isinstance(start, int) and start < 0) or (isinstance(stop, int) and stop < 0):
...
def _is_mapped_annotation(
    ...
    try:
        annotated = de_stringify_annotation(cls, raw_annotation, originating_cls.__module__)
    except NameError:
...
def _cleanup_mapped_str_annotation(annotation: str, originating_module: str) -> str:
    ...
    if (
        ...
        stripchars = "\"' "
        stack[-1] = ", ".join(f'"{elem.strip(stripchars)}"' for elem in stack[-1].split(","))
