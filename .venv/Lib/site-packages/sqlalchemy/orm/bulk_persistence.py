    ...
    *,
    ...
    if session_transaction.session.connection_callable:
        raise NotImplementedError(
            "connection_callable / per-instance sharding " "not supported in bulk_insert()"
        )
    ...
    for table, super_mapper in mappers_to_run:
        ...
        extra_bp_names = (
            [b.key for b in use_orm_insert_stmt._get_embedded_bindparams() if b.key in mappings[0]]
...
def _bulk_update(
    ...
    *,
    ...
    def _changed_dict(mapper, state):
        return {
            k: v for k, v in state.dict.items() if k in state.committed_state or k in search_keys
        }
    ...
    if session_transaction.session.connection_callable:
        raise NotImplementedError(
            "connection_callable / per-instance sharding " "not supported in bulk_update()"
        )
    ...
    extra_bp_names = (
        [b.key for b in use_orm_update_stmt._get_embedded_bindparams() if b.key in mappings[0]]
        ...
        records = persistence._collect_update_commands(
            ...
            (
                (
                    ...
                    mapper,
                    connection,
                    (mapping[mapper._version_id_prop.key] if mapper._version_id_prop else None),
                )
...
class ORMDMLState(AbstractORMCompileState):
    ...
    def get_returning_column_descriptions(cls, statement):
        ...
        return [
            ...
            for c, ent in [(c, _ent_for_col(c)) for c in statement._all_selected_columns]
        ]
    ...
    def _setup_orm_returning(
        ...
        *,
        ...
        if orm_level_statement._returning:
            ...
            self.from_statement_ctx = fsc = ORMFromStatementCompileState.create_for_statement(
                fs, compiler
            )
...
class BulkUDCompileState(ORMDMLState):
    ...
    def orm_pre_session_exec(
        ...
        if "parententity" not in statement.table._annotations:
            ...
            if update_options._dml_strategy == "auto":
                ...
                raise sa_exc.InvalidRequestError(
                    'Can\'t use "bulk" ORM insert strategy without ' "passing separate parameters"
                )
        else:
            if update_options._dml_strategy == "auto":
        ...
        return (
            statement,
            util.immutabledict(execution_options).union({"_sa_orm_update_options": update_options}),
        )
    ...
    def _adjust_for_extra_criteria(cls, global_attributes, ext_info):
        ...
        if (
            ...
            return_crit += tuple(
                ...
                for ae in global_attributes[("additional_entity_criteria", ext_info.mapper)]
    ...
    def _resolved_keys_as_propnames(cls, mapper, resolved_values):
        ...
        for k, v in resolved_values:
            if mapper and isinstance(k, expression.ColumnElement):
                ...
                raise sa_exc.InvalidRequestError(
                    "Attribute name not found, can't be " "synchronized back to objects: %r" % k
                )
        return values
...
class BulkORMInsert(ORMDMLState, InsertDMLState):
    ...
    def orm_pre_session_exec(
        ...
        return (
            statement,
            util.immutabledict(execution_options).union({"_sa_orm_insert_options": insert_options}),
        )
    ...
    def orm_execute_statement(
        ...
        if insert_options._dml_strategy not in (
            ...
            raise sa_exc.ArgumentError(
                "Valid strategies for ORM insert strategy " "are 'raw', 'orm', 'bulk', 'auto"
            )
    ...
    def _resolved_keys_as_col_keys(cls, mapper, resolved_value_dict):
        return {
            ...
            for col, k, v in ((mapper.c.get(k), k, v) for k, v in resolved_value_dict.items())
        }
...
class BulkORMUpdate(BulkUDCompileState, UpdateDMLState):
    ...
    def _setup_for_orm_update(self, statement, compiler, **kw):
        ...
        if can_use_returning is not False:
            ...
            can_use_returning = synchronize_session == "fetch" and self.can_use_returning(
    ...
    def orm_execute_statement(
        ...
        if update_options._dml_strategy not in (
            ...
            raise sa_exc.ArgumentError(
                "Valid strategies for ORM UPDATE strategy " "are 'orm', 'auto', 'bulk', 'core_only'"
            )
    ...
    def _do_post_synchronize_fetch(cls, session, statement, result, update_options):
        ...
        if returned_defaults_rows:
            pk_rows = cls._interpret_returning_rows(result, target_mapper, returned_defaults_rows)
        ...
        objs = [
            ...
            for identity_key in [
                ...
                for primary_key, identity_token in [(row[0:-1], row[-1]) for row in matched_rows]
    ...
    def _apply_update_set_values_to_objects(
        ...
        to_expire = {
            mapper._columntoproperty[c].key for c in postfetch_cols if c in mapper._columntoproperty
        }.difference(evaluated_keys)

        prefetch_transfer = [(mapper._columntoproperty[c].key, c.key) for c in to_prefetch]

        for obj, state, dict_ in matched_objects:

            dict_.update(
                {col_to_prop: effective_params[c_key] for col_to_prop, c_key in prefetch_transfer}
            )
...
class BulkORMDelete(BulkUDCompileState, DeleteDMLState):
    ...
    def orm_execute_statement(
        ...
        if update_options._dml_strategy not in ("orm", "auto", "core_only"):
            raise sa_exc.ArgumentError(
                "Valid strategies for ORM DELETE strategy are 'orm', 'auto', " "'core_only'"
            )
