...
_composite_getters: weakref.WeakKeyDictionary[Type[Any], Callable[[Any], Tuple[Any, ...]]] = (
...
class CompositeProperty(_MapsColumns[_CC], _IntrospectsAnnotations, DescriptorProperty[_CC]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.comparator_factory = (
            comparator_factory if comparator_factory is not None else self.__class__.Comparator
        )
    ...
    def _populate_composite_bulk_save_mappings_fn(
        ...
        def populate(dest_dict: Dict[str, Any]) -> None:
            dest_dict.update(
                {key: val for key, val in zip(attrs, get_values(dest_dict.pop(self.key)))}
            )
    ...
    class Comparator(PropComparator[_PT]):
        ...
        def _bulk_update_tuples(self, value: Any) -> Sequence[Tuple[_DMLColumnArgument, Any]]:
            ...
            if value is None:
                ...
                values = self.prop._composite_values_from_instance(value)  # type: ignore[arg-type]
            else:
                raise sa_exc.ArgumentError(
    ...
    def _proxied_object(
        ...
        if not hasattr(attr, "property") or not isinstance(attr.property, MapperProperty):
            ...
            raise sa_exc.InvalidRequestError(
                ...
                """ORM mapped attributes, got %r""" % (self.parent.class_.__name__, self.name, attr)
            )
    ...
    def set_parent(self, parent: Mapper[Any], init: bool) -> None:
        ...
        if self.map_column:
            ...
            if self.key not in parent.persist_selectable.c:
                ...
                and parent._columntoproperty[parent.persist_selectable.c[self.key]].key == self.name
            ):
                raise sa_exc.ArgumentError(
