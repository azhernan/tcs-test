...
class SessionTransaction(_StateChange, TransactionalContext):
    ...
    def prepare(self) -> None:
        if self._parent is not None or not self.session.twophase:
            raise sa_exc.InvalidRequestError(
                "'twophase' mode not enabled, or not root transaction; " "can't prepare."
            )
        self._prepare_impl()
...
class Session(_SessionClassMethods, EventTarget):
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            and join_transaction_mode not in JoinTransactionMode.__args__  # type: ignore
        ):
            raise sa_exc.ArgumentError(
                f"invalid selection for join_transaction_mode: " f'"{join_transaction_mode}"'
            )
    ...
    def _autobegin_t(self, begin: bool = False) -> SessionTransaction:
        if self._transaction is None:
            ...
            trans = SessionTransaction(
                self,
                (SessionTransactionOrigin.BEGIN if begin else SessionTransactionOrigin.AUTOBEGIN),
            )
    ...
    def begin(self, nested: bool = False) -> SessionTransaction:
        ...
        if nested:
            ...
            raise sa_exc.InvalidRequestError("A transaction is already begun on this Session.")
    ...
    def get_bind(
        ...
        **kw: Any,
        ...
        raise sa_exc.UnboundExecutionError(
            f"Could not locate a bind configured on " f'{", ".join(context)} or this Session.'
        )
    ...
    def _expire_state(
        ...
        if attribute_names:
            ...
            # remove associations
            cascaded = list(state.manager.mapper.cascade_iterator("refresh-expire", state))
    ...
    def _get_impl(
        ...
        *,
        ...
        if mapper is None or not mapper.is_mapper:
            raise sa_exc.ArgumentError("Expected mapped class or mapper, got: %r" % entity)
        ...
        if is_dict:
            ...
            try:
                primary_key_identity = list(
                    primary_key_identity[prop.key] for prop in mapper._identity_key_props
                )
        ...
        if not populate_existing and not mapper.always_refresh and with_for_update is None:
    ...
    def _update_impl(self, state: InstanceState[Any], revert_deletion: bool = False) -> None:
        if state.key is None:
    ...
    def _is_clean(self) -> bool:
        return not self.identity_map.check_modified() and not self._deleted and not self._new
    ...
    def _flush(self, objects: Optional[Sequence[object]] = None) -> None:
        ...
        for state in proc:
            ...
            if is_orphan and not is_persistent_orphan and state._orphaned_outside_of_session:
                ...
                _reg = flush_context.register_object(state, isdelete=is_persistent_orphan)
    ...
    def is_modified(self, instance: object, include_collections: bool = True) -> bool:
        ...
        * Scalar attributes may not have recorded the previously set
        ...
        for attr in state.manager.attributes:
            if (not include_collections and hasattr(attr.impl, "get_collection")) or not hasattr(
                attr.impl, "get_history"
            ):
                continue
