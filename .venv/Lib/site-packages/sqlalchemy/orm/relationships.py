            ...
            if other is None or isinstance(other, expression.Null):
                if self.property.direction in [ONETOMANY, MANYTOMANY]:
                    ...
                    return _orm_annotate(
                        self.property._optimized_compare(None, adapt_source=self.adapter)
                    )
            ...
            if current_value is LoaderCallableStatus.NEVER_SET:
                if not existing_is_available:
                    raise sa_exc.InvalidRequestError(
                        ...
                        "%s; no value has been set for this column" % (column, state_str(state))
                    )
    ...
    def _add_reverse_property(self, key: str) -> None:
        ...
        if (
            ...
            raise sa_exc.ArgumentError(
                ...
                "set remote_side on the many-to-one side ?" % (other, self, self.direction)
            )
    ...
    def _check_cascade_settings(self, cascade: CascadeOptions) -> None:
        if (
            ...
            raise sa_exc.ArgumentError(
                ...
                % {
                    "rel": self,
                    "direction": ("many-to-one" if self.direction is MANYTOONE else "many-to-many"),
    ...
    def _determine_direction(self) -> None:
        ...
        if self.secondaryjoin is not None:
            ...
            if onetomany_fk and manytoone_fk:
                ...
                manytoone_local = {
                    c
                    for c in self._gather_columns_with_annotation(self.primaryjoin, "foreign")
