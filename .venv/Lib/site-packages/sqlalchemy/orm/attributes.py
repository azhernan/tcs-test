...
class AttributeImpl:
    ...
    def _default_value(self, state: InstanceState[Any], dict_: _InstanceDict) -> Any:
        ...
        assert self.key not in dict_, (
            "_default_value should only be invoked for an " "uninitialized or expired attribute"
        )
...
class ScalarObjectAttributeImpl(ScalarAttributeImpl):
    ...
    def get_history(
        ...
        if not self._deferred_history:
            ...
            return History.from_object_attribute(self, state, current, original=original)
    ...
    def get_all_pending(
        ...
        if current is not None and current is not PASSIVE_NO_RESULT and current is not NO_VALUE:
        ...
        if _adapt:
            if new_collection._converter is not None:
                ...
                if setting_type is not receiving_type:
                    ...
                    raise TypeError(
                        "Incompatible collection type: %s is not %s-like" % (given, wanted)
                    )
...
def backref_listeners(attribute: QueryableAttribute[Any], key: str, uselist: bool) -> None:
    ...
    def emit_backref_from_collection_append_event(state, child, initiator, **kw):
        ...
        check_append_token = child_impl._append_token
        check_bulk_replace_token = (
            child_impl._bulk_replace_token if _is_collection_attribute_impl(child_impl) else None
        )
...
def register_attribute_impl(
    ...
    **kw: Any,
    ...
    dispatch = cast("_Dispatch[QueryableAttribute[Any]]", manager[key].dispatch)  # noqa: E501
    ...
    if impl_class:
        ...
        impl = cast("Type[WriteOnlyAttributeImpl]", impl_class)(class_, key, dispatch, **kw)
    elif uselist:
