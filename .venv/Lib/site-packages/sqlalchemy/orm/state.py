...
class InstanceState(interfaces.InspectionAttrInfo, Generic[_O]):
    ...
    def attrs(self) -> util.ReadOnlyProperties[AttributeState]:
        ...
        """
        return util.ReadOnlyProperties({key: AttributeState(self, key) for key in self.manager})
    ...
    def _attached(self) -> bool:
        return (
            self.session_id is not None and self.session_id in util.preloaded.orm_session._sessions
        )
    ...
    def _unloaded_non_object(self) -> Set[str]:
        return self.unloaded.intersection(
            attr for attr in self.manager if self.manager[attr].impl.accepts_scalar_loader
        )
