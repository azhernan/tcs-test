...
class CollectionAdapter:
    ...
    def append_with_event(self, item: Any, initiator: Optional[AttributeEventToken] = None) -> None:
    ...
    def remove_with_event(self, item: Any, initiator: Optional[AttributeEventToken] = None) -> None:
...
def bulk_replace(values, existing_adapter, new_adapter, initiator=None):
    ...
    if existing_adapter:
        existing_adapter._fire_append_wo_mutation_event_bulk(constants, initiator=initiator)
...
def _assert_required_roles(cls, roles, methods):
    ...
    if "appender" not in roles or not hasattr(cls, roles["appender"]):
        raise sa_exc.ArgumentError(
            "Type %s must elect an appender method to be " "a collection class" % cls.__name__
        )
    ...
    if "remover" not in roles or not hasattr(cls, roles["remover"]):
        raise sa_exc.ArgumentError(
            "Type %s must elect a remover method to be " "a collection class" % cls.__name__
        )
    ...
    if "iterator" not in roles or not hasattr(cls, roles["iterator"]):
        raise sa_exc.ArgumentError(
            "Type %s must elect an iterator method to be " "a collection class" % cls.__name__
        )
...
def _set_collection_attributes(cls, roles, methods):
    ...
    for method_name, (before, argument, after) in methods.items():
        setattr(
            cls,
            method_name,
            _instrument_membership_mutator(getattr(cls, method_name), before, argument, after),
        )
    # intern the role map
...
__canned_instrumentation: util.immutabledict[Any, _CollectionFactoryType] = util.immutabledict(
    {
