...
class Mapper(
    ...
    def __init__(
        ...
        version_id_generator: Optional[Union[Literal[False], Callable[[Any], Any]]] = None,
        ...
        if inherit_condition is not None:
            self.inherit_condition = coercions.expect(roles.OnClauseRole, inherit_condition)
        else:
        ...
        with _CONFIGURE_MUTEX:
            cast("MapperEvents", self.dispatch._events)._new_mapper_instance(class_, self)
    ...
    def _prefer_eager_defaults(self, dialect, table):
        if self.eager_defaults == "auto":
            ...
            return table in self._server_default_col_keys and dialect.insert_executemany_returning
        else:
    ...
    def _configure_inheritance(self):
        ...
        if self.inherits:
            ...
            if self.polymorphic_load and self.concrete:
                raise sa_exc.ArgumentError(
                    "polymorphic_load is not currently supported " "with concrete table inheritance"
                )
                    ...
                    for name in method.__sa_validators__:
                        if name in self.validators:
                            raise sa_exc.InvalidRequestError(
                                ...
                                "attribute %r on mapper %s already exists." % (name, self)
                            )
    ...
    def _configure_pks(self) -> None:
        ...
        if self._primary_key_argument:
            coerced_pk_arg = [
                (self._str_arg_to_mapped_col("primary_key", c) if isinstance(c, str) else c)
                for c in (
                    coercions.expect(
        ...
        # otherwise, see that we got a full PK for the mapped table
            ...
            raise sa_exc.ArgumentError(
                ...
                "key columns for mapped table '%s'" % (self, self.persist_selectable.description)
            )
            ...
            util.warn(
                ...
                "no rows will be persisted in this Table." % self.local_table.description
            )
        ...
        if self.inherits and not self.concrete and not self._primary_key_argument:
            ...
            if coerced_pk_arg:
                primary_key = [
                    ...
                    for cc, c in (
                        (self.persist_selectable.corresponding_column(c), c) for c in coerced_pk_arg
                    )
                ]
            else:
            ...
            if self.polymorphic_on in self._columntoproperty:
                ...
                if key:
                    if self._should_exclude(key, key, False, col):
                        raise sa_exc.InvalidRequestError(
                            "Cannot exclude or override the " "discriminator column %r" % key
                        )
                else:
        ...
        if setter:
            ...
            def _validate_polymorphic_identity(mapper, state, dict_):
                if (
                    ...
                    and dict_[polymorphic_key] not in mapper._acceptable_polymorphic_identities
                ):
                    util.warn_limited(
    ...
    def _configure_property(
        ...
        *,
        ...
        if (
            key in self._props
            and not isinstance(self._props[key], descriptor_props.ConcreteInheritedProperty)
    ...
    def add_property(self, key: str, prop: Union[Column[Any], MapperProperty[Any]]) -> None:
        ...
        """
        prop = self._configure_property(key, prop, init=self.configured, warn_for_existing=True)
    ...
    def _single_table_criterion(self):
        if self.single and self.inherits and self.polymorphic_on is not None:
            return self.polymorphic_on._annotate({"parententity": self, "parentmapper": self}).in_(
                [
    ...
    def _with_polymorphic_selectable(self) -> FromClause:
        ...
        if selectable is not None:
            ...
            return self._selectable_from_mappers(self._mappers_from_spec(spec, selectable), False)
    ...
    def _pk_keys_by_table(self):
        return {
            table: frozenset([col.key for col in pks]) for table, pks in self._pks_by_table.items()
        }
    ...
    def _server_default_plus_onupdate_propkeys(self) -> Set[str]:
        ...
        for table, columns in self._server_default_cols.items():
            result.update(col_to_property[col].key for col in columns.intersection(col_to_property))
    ...
    def _with_polymorphic_args(
        ...
        if selectable not in (None, False):
            selectable = coercions.expect(roles.StrictFromClauseRole, selectable, allow_select=True)
    ...
    def _polymorphic_properties(self):
        return list(self._iterate_polymorphic_properties(self._with_polymorphic_mappers))
    ...
    def _all_column_expressions(self):
        ...
        return [
            ...
            if isinstance(prop, properties.ColumnProperty) and prop._renders_in_subqueries
    ...
    def relationships(
        ...
        """
        return self._filter_properties(util.preloaded.orm_relationships.RelationshipProperty)
    ...
    def composites(self) -> util.ReadOnlyProperties[CompositeProperty[Any]]:
        ...
        """
        return self._filter_properties(util.preloaded.orm_descriptor_props.CompositeProperty)
    ...
    def is_sibling(self, other: Mapper[Any]) -> bool:
        ...
        """
        return self.base_mapper is other.base_mapper and not self.isa(other) and not other.isa(self)
    ...
    def _get_committed_state_attr_by_column(
        ...
        prop = self._columntoproperty[column]
        return state.manager[prop.key].impl.get_committed_value(state, dict_, passive=passive)
    ...
    def _subclass_load_via_in(self, entity, polymorphic_from):
        ...
        if entity.is_aliased_class:
            ...
            q = q.where(in_expr.in_(sql.bindparam("primary_keys", expanding=True))).order_by(
                *primary_key
            )
        else:
    ...
    def cascade_iterator(
        ...
        visitables = deque([(deque(state.mapper._props.values()), prp, state, state.dict)])
...
def _do_configure_registries(registries: Set[_RegistryType], cascade: bool) -> None:
    ...
    for reg in registry._recurse_with_dependencies(registries):
        ...
        if not cascade and reg._dependencies.difference(orig):
            raise sa_exc.InvalidRequestError(
                "configure was called with cascade=False but " "additional registries remain"
            )
    ...
    def __missing__(self, column):
        ...
        if prop:
            raise orm_exc.UnmappedColumnError(
                ...
                "conflicting property '%s':%r" % (column.table.name, column.name, column.key, prop)
            )
        raise orm_exc.UnmappedColumnError(
