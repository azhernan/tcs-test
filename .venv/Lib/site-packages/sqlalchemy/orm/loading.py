    ...
    unique_filters = [
        (
            ...
            if context.yield_per
                ...
                if (not ent.use_id_for_hash and (ent._non_hashable_value or ent._null_column_type))
...
def merge_result(
    ...
    ctx = querycontext.ORMSelectCompileState._create_entities_collection(query, legacy=True)
...
def _load_subclass_via_in(context, path, entity, polymorphic_from, option_entities):
    ...
    def do_load(context, path, states, load_only, effective_entity):
        ...
        context.session.execute(
            ...
            dict(
                primary_keys=[
                    state.key[1][0] if zero_idx else state.key[1] for state, load_attrs in states
                ]
            ),
        ).unique().scalars().all()
...
def load_scalar_attributes(mapper, state, attribute_names, passive):
    ...
    if (_none_set.issubset(identity_key) and not mapper.allow_partial_pks) or _none_set.issuperset(
        identity_key
    ):
        util.warn_limited(
