...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def joinedload(
        ...
        loader = self._set_relationship_strategy(
            attr,
            {"lazy": "joined"},
            opts=({"innerjoin": innerjoin} if innerjoin is not None else util.EMPTY_DICT),
        )
        return loader
...
class Load(_AbstractLoad):
    ...
    def _apply_to_parent(self, parent: Load) -> None:
        ...
        if not any(
            orm_util._entity_corresponds_to_use_path_impl(elem, cloned.path.odd_element(0))
            for elem in (parent.path.odd_element(-1),) + parent.additional_source_entities
        ):
            ...
            try:
                ...
                if not isinstance(opt, _AbstractLoad):
                    raise sa_exc.ArgumentError(
                        f"Loader option {opt} is not compatible with the " "Load.options() method."
                    ) from ae
                else:
                    raise
        return self
    ...
    def _clone_for_bind_strategy(
        ...
        if self.path.is_token:
            ...
            if strategy:
                ...
                raise sa_exc.ArgumentError(
                    f"Mapped attribute '{self.path.prop}' does not " "refer to a mapped entity"
                )
...
class _LoadElement(cache_key.HasCacheKey, traversals.HasShallowCopy, visitors.Traversible):
    ...
    def _prepend_path_from(self, parent: Load) -> _LoadElement:
        ...
        if not any(
            ...
            raise sa_exc.ArgumentError(
                f'Attribute "{self.path[1]}" does not link ' f'from element "{parent.path[-1]}".'
            )
...
def _raise_for_does_not_link(path, attrname, parent_entity):
    if len(path) > 1:
        ...
        raise sa_exc.ArgumentError(
            ...
            % (
                ...
                (
                    ...
                    and orm_util._entity_corresponds_to(path.entity, inspect(parent_entity).mapper)
                    else ""
                ),
            )
        )
        ...
        raise sa_exc.ArgumentError(
            f'ORM mapped attribute "{attrname}" does not ' f'link mapped class "{path[-1]}"'
        )
