...
class _MultipleClassMarker(ClsRegistryToken):
    ...
    def attempt_get(self, path: List[str], key: str) -> Type[Any]:
        if len(self.contents) > 1:
            raise exc.InvalidRequestError(
                ...
                "base. Please use a fully module-qualified path." % (".".join(path + [key]))
            )
        else:
    ...
    def add_item(self, item: Type[Any]) -> None:
        ...
        modules = {
            cls.__module__ for cls in [ref() for ref in list(self.contents)] if cls is not None
        }
        if item.__module__ in modules:
            util.warn(
                "This declarative base already contains a class with the "
                "same class name and module name as %s.%s, and will "
                "be replaced in the string-lookup table." % (item.__module__, item.__name__)
            )
