    ...
    for state, state_dict, mapper, connection in states_to_insert:
        ...
        for propkey in set(propkey_to_col).intersection(state_dict):
            ...
            if value is None and col not in eval_none and not render_nulls:
                ...
                hasattr(value, "__clause_element__") or isinstance(value, sql.ClauseElement)
            ):
                value_params[col] = (
                    value.__clause_element__() if hasattr(value, "__clause_element__") else value
                )
            else:
        ...
        if bulk:
            ...
            for propkey in set(propkey_to_col).intersection(state.committed_state):
                ...
                # objects for __eq__()
                    ...
                    state.manager[propkey].impl.is_equal(value, state.committed_state[propkey])
                    is not True
                ):
                    params[col.key] = value

            if mapper.base_mapper.eager_defaults is True:
                has_all_defaults = (mapper._server_onupdate_default_col_keys[table]).issubset(
                    params
                )
            else:
                has_all_defaults = True
...
def _emit_update_statements(
    ...
    *,
    ...
    needs_version_id = (
        mapper.version_id_col is not None and mapper.version_id_col in mapper._cols_by_table[table]
    )
        ...
        if check_rowcount:
            if rows != len(records):
                raise orm_exc.StaleDataError(
                    ...
                    "update %d row(s); %d were matched." % (table.description, len(records), rows)
                )
    ...
    *,
    ...
    if use_orm_insert_stmt is not None:
        ...
        deterministic_results_reqd = (
            returning_is_required_anyway and use_orm_insert_stmt._sort_by_parameter_order
        ) or bookkeeping
    else:
                ...
                for (
                    ...
                    if state:
                        _postfetch(
                            ...
                            value_params,
                            False,
                            (result.returned_defaults if not result.context.executemany else None),
                        )
                    else:
...
def _emit_delete_statements(base_mapper, uowtransaction, mapper, table, delete):
    ...
    for connection, recs in groupby(delete, lambda rec: rec[1]):  # connection
        ...
        if need_version_id and not connection.dialect.supports_sane_multi_rowcount:
            if connection.dialect.supports_sane_rowcount:
                ...
                util.warn(
                    ...
                    "- versioning cannot be verified." % connection.dialect.dialect_description
                )
        ...
        if (
            ...
            and (connection.dialect.supports_sane_multi_rowcount or len(del_objects) == 1)
        ):
        ...
        if mapper._readonly_props:
            readonly = state.unmodified_intersection(
                [
                    ...
                    or (not p.expire_on_flush and not p.deferred and p.key not in state.dict)
                ]
            )
            if readonly:
...
def _postfetch(
    ...
    if isupdate and value_params:
        ...
        postfetch_cols.extend(
            [col for col in value_params if col.primary_key and col not in returning_cols]
        )
