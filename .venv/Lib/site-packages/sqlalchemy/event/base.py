    ...
    def __call__(self, _instance_cls: Type[_ET]) -> _Dispatch[_ET]:
        for cls in _instance_cls.__mro__:
            if "dispatch" in cls.__dict__:
                return cast("_Dispatch[_ET]", cls.__dict__["dispatch"].dispatch)._for_class(
                    _instance_cls
                )
        else:
    ...
    def __init__(
        ...
        if instance_cls:
            ...
            try:
                ...
                self._empty_listeners = self._empty_listener_reg[instance_cls] = {
                    ls.name: _EmptyListener(ls, instance_cls) for ls in parent._event_descriptors
                }
        else:
...
class Events(_HasEventsDispatch[_ET]):
    ...
    def _accept_with(
        ...
        def dispatch_parent_is(t: Type[Any]) -> bool:
            return isinstance(cast("_JoinedDispatcher[_ET]", target.dispatch).parent, t)
        ...
        if hasattr(target, "dispatch"):
            if (
                ...
                or (dispatch_is(_JoinedDispatcher) and dispatch_parent_is(cls.dispatch.__class__))
            ):
                return target
    ...
    def _listen(
        ...
        *,
        ...
        event_key.base_listen(propagate=propagate, insert=insert, named=named, asyncio=asyncio)
