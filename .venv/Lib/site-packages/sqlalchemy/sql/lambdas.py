...
class LambdaElement(elements.ClauseElement):
    ...
    def _retrieve_tracker_rec(self, fn, apply_propagate_attrs, opts):
        ...
        if parent_closure_cache_key is not _cache_key.NO_CACHE:
            ...
            cache_key = tuple(
                [getter(closure, opts, anon_map, bindparams) for getter in tracker.closure_trackers]
            )
...
class AnalyzedCode:
    ...
    def _init_track_on(self, track_on):
        self.closure_trackers.extend(
            self._cache_key_getter_track_on(idx, elem) for idx, elem in enumerate(track_on)
        )
    ...
    def _cache_key_getter_track_on(self, idx, elem):
        ...
        if isinstance(elem, tuple):
            ...
            def get(closure, opts, anon_map, bindparams):
                return tuple(
                    tup_elem._gen_cache_key(anon_map, bindparams) for tup_elem in opts.track_on[idx]
                )
...
class AnalyzedFunction:
    ...
    def _instrument_and_run_function(self, lambda_element):
        ...
        if not build_py_wrappers:
            ...
            if closure:
                new_closure = {
                    fv: cell.cell_contents for fv, cell in zip(fn.__code__.co_freevars, closure)
                }
            else:
                new_closure = {}
