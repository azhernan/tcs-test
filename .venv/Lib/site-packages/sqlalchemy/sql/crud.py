...
_CrudParamElement = Tuple[
    "ColumnElement[Any]",
    str,  # column name
    Optional[Union[str, "_SQLExprDefault"]],  # bound parameter string or SQL expression to apply
    Iterable[str],
]
...
def _get_crud_params(
    ...
    **kw: Any,
    ...
    if compiler.column_keys is None:
        ...
        parameters = {
            _column_as_key(key): REQUIRED for key in compiler.column_keys if key not in spd_str_key
        }
    else:
    ...
    if (
        ...
        and (
            ...
            and (
                ...
                (c.default is None and compiler.dialect.preexecute_autoincrement_sequences)
            )
        )
    ):
...
def _get_update_multitable_params(
    ...
    normalized_params = {
        coercions.expect(roles.DMLColumnRole, c): param for c, param in stmt_parameter_tuples or ()
    }
...
def _get_returning_modifiers(compiler, stmt, compile_state, toplevel):
    ...
    postfetch_lastrowid = (
        need_pks and dialect.postfetch_lastrowid and stmt.table._autoincrement_column is not None
    )
    ...
    if _compile_state_isinsert(compile_state):
        ...
        explicit_returning = (
            should_implicit_return_defaults or stmt._returning or stmt._supplemental_returning
        )
        use_insertmanyvalues = (
            toplevel
