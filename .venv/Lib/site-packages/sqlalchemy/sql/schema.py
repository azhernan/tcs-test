...
class Table(DialectKWArgs, HasSchemaAttr, TableClause, inspection.Inspectable["Table"]):
    ...
    def _new(cls, *args: Any, **kw: Any) -> Any:
        ...
        try:
            ...
            raise TypeError(
                "Table() takes at least two positional-only " "arguments 'name' and 'metadata'"
            )
    ...
    def _init_existing(self, *args: Any, **kwargs: Any) -> None:
        ...
        if schema and schema != self.schema:
            raise exc.ArgumentError(
                f"Can't change schema of existing table " f"from '{self.schema}' to '{schema}'",
            )
    ...
    @util.deprecated(
        "1.4",
        ":meth:`_schema.Table.tometadata` is renamed to " ":meth:`_schema.Table.to_metadata`",
    )
    def tometadata(
        self,
        metadata: MetaData,
    ...
    def to_metadata(
        ...
        for index in self.indexes:
            ...
            Index(
                index.name,
                unique=index.unique,
                *[_copy_expression(expr, self, table) for expr in index._table_bound_expressions],
                _table=table,
                **index.kwargs,
            )
...
class Column(DialectKWArgs, SchemaItem, ColumnClause[_T]):
    ...
    def __init__(
        self,
        __name_pos: Optional[Union[str, _TypeEngineArgument[_T], SchemaEventTarget]] = None,
        ...
        if l_args:
            ...
            if hasattr(coltype, "_sqla_type"):
                if type_ is not None:
                    raise exc.ArgumentError("May not pass type_ positionally and as a keyword.")
    ...
    def __repr__(self) -> str:
        ...
        return "Column(%s)" % ", ".join(
            ...
            + [(self.table is not None and "table=<%s>" % self.table.description or "table=None")]
    ...
    def _set_parent(  # type: ignore[override]
        ...
        **kw: Any,
        ...
        if existing is not None and existing is not table:
            raise exc.ArgumentError(
                f"Column object '{self.key}' already " f"assigned to Table '{existing.description}'"
            )
        ...
        if self._insert_sentinel:
            if self.table._sentinel_column is not None:
                raise exc.ArgumentError("a Table may have only one explicit sentinel column")
    ...
    def _copy(self, **kw: Any) -> Column[Any]:
        ...
        args: List[SchemaItem] = [c._copy(**kw) for c in self.constraints if not c._type_bound] + [
        ...
        for dialect_name in self.dialect_options:
            ...
            for (
                ...
                column_kwargs[dialect_name + "_" + dialect_option_key] = dialect_option_value
    ...
    def _make_proxy(
        ...
        **kw: Any,
        ...
        fk = [
            ...
            for f, col in [(fk, fk._resolve_column(raiseerr=False)) for fk in self.foreign_keys]
        ]
        ...
        try:
            c = self._constructor(
                ...
                _proxies=(list(compound_select_cols) if compound_select_cols else [self]),
                *fk,
            )
...
class ForeignKey(DialectKWArgs, SchemaItem):
    ...
    def _column_tokens(self) -> Tuple[Optional[str], str, Optional[str]]:
        ...
        if m is None:
            raise exc.ArgumentError(f"Invalid foreign key column specification: {self._colspec}")
        if len(m) == 1:
            tname = m.pop()
            colname = None
        else:
        ...
        if isinstance(self._colspec, str):
            ...
            if self._unresolvable or tablekey not in parenttable.metadata:
                ...
                if not raiseerr:
                    return None
                raise exc.InvalidRequestError(
                    f"Table {parenttable} is no longer associated with its " "parent MetaData"
                )
            else:
...
class ColumnCollectionMixin:
        ...
        *columns: _DDLColumnArgument,
        ...
        if processed_expressions is not None:
            ...
            for (
                ...
                _,
                add_element,
            ) in coercions.expect_col_expression_collection(roles.DDLConstraintColumnRole, columns):
                self._pending_colargs.append(add_element)
                processed_expressions.append(expr)
        else:
            self._pending_colargs = [
                coercions.expect(roles.DDLConstraintColumnRole, column) for column in columns
            ]
    ...
    def _check_attach(self, evt: bool = False) -> None:
        ...
        if cols_wo_table:
            ...
            has_string_cols = {c for c in self._pending_colargs if c is not None}.difference(
                col_objs
            )
        ...
        if len(tables) == 1:
            ...
            if others:
                ...
                raise exc.ArgumentError(
                    f"Column(s) {other_str} " f"are not part of table '{table.description}'."
                )
    ...
    def _col_expressions(self, parent: Union[Table, Column[Any]]) -> List[Optional[Column[Any]]]:
        if isinstance(parent, Column):
            ...
            try:
                return [
                    parent.c[col] if isinstance(col, str) else col for col in self._pending_colargs
                ]
    ...
    def _copy(
        ...
        **kw: Any,
        ...
        c = self.__class__(
            ...
            deferrable=self.deferrable,
            initially=self.initially,
            *[_copy_expression(expr, self.parent, target_table) for expr in self._columns],
            comment=self.comment,
            **constraint_kwargs,
        )
        return self._schema_item_copy(c)
...
class CheckConstraint(ColumnCollectionConstraint):
    ...
    def copy(self, *, target_table: Optional[Table] = None, **kw: Any) -> CheckConstraint:
        return self._copy(target_table=target_table, **kw)

    def _copy(self, *, target_table: Optional[Table] = None, **kw: Any) -> CheckConstraint:
...
class MetaData(HasSchemaAttr):
    ...
    def __init__(
        ...
        self._fk_memos: Dict[Tuple[str, Optional[str]], List[ForeignKey]] = collections.defaultdict(
            list
        )
    ...
    def _remove_table(self, name: str, schema: Optional[str]) -> None:
        ...
        if self._schemas:
            self._schemas = {t.schema for t in self.tables.values() if t.schema is not None}
    ...
    def sorted_tables(self) -> List[Table]:
        ...
        """
        return ddl.sort_tables(sorted(self.tables.values(), key=lambda t: t.key))  # type: ignore
    ...
    def reflect(
        ...
        schema: Optional[str] = None,
        views: bool = False,
        only: Union[_typing_Sequence[str], Callable[[str, MetaData], bool], None] = None,
        extend_existing: bool = False,
        autoload_replace: bool = True,
        resolve_fks: bool = True,
        **dialect_kwargs: Any,
        ...
        with inspection.inspect(bind)._inspection_context() as insp:
            ...
            available: util.OrderedSet[str] = util.OrderedSet(insp.get_table_names(schema))
            if views:
    ...
    def create_all(
        ...
        """
        bind._run_ddl_visitor(ddl.SchemaGenerator, self, checkfirst=checkfirst, tables=tables)
    ...
    def drop_all(
        ...
        """
        bind._run_ddl_visitor(ddl.SchemaDropper, self, checkfirst=checkfirst, tables=tables)
...
class Computed(FetchedValue, SchemaItem):
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        ...
        if not isinstance(parent.server_default, (type(None), Computed)) or not isinstance(
            parent.server_onupdate, (type(None), Computed)
        ):
            raise exc.ArgumentError(
...
class Identity(IdentityOptions, FetchedValue, SchemaItem):
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        assert isinstance(parent, Column)
        if not isinstance(parent.server_default, (type(None), Identity)) or not isinstance(
            parent.server_onupdate, type(None)
        ):
            raise exc.ArgumentError(
