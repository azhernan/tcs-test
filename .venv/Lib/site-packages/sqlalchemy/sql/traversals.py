...
class HasShallowCopy(HasTraverseInternals):
    ...
    def _generate_shallow_copy(
        ...
        code = "\n".join(
            f"    other.{attrname} = self.{attrname}" for attrname, _ in internal_dispatch
        )
    ...
    def _generate_shallow_from_dict(
        ...
        code = "\n".join(
            f"    self.{attrname} = d['{attrname}']" for attrname, _ in internal_dispatch
        )
...
class _CopyInternalsTraversal(HasTraversalDispatch):
    ...
    def visit_dml_values(self, attrname, parent, element, clone=_clone, **kw):
        return {
            (clone(key, **kw) if hasattr(key, "__clause_element__") else key): clone(value, **kw)
    ...
    def visit_dml_multi_values(self, attrname, parent, element, clone=_clone, **kw):
        ...
        def copy(elem):
            if isinstance(elem, (list, tuple)):
                return [
                    (clone(value, **kw) if hasattr(value, "__clause_element__") else value)
                    for value in elem
                ]
...
class TraversalComparatorStrategy(HasTraversalDispatch, util.MemoizedSlots):
        ...
        while stack:
            ...
            for (
                ...
                if not compare_annotations and (
                    (left_attrname == "_annotations") or (right_attrname == "_annotations")
                ):
                    continue
    ...
    def visit_executable_options(self, attrname, left_parent, left, right_parent, right, **kw):
        for l, r in zip_longest(left, right, fillvalue=None):
            ...
            if (l._gen_cache_key(self.anon_map[0], []) if l._is_has_cache_key else l) != (
    ...
    def visit_dml_multi_values(self, attrname, left_parent, left, right_parent, right, **kw):
        for lseq, rseq in zip_longest(left, right, fillvalue=None):
            ...
            for ld, rd in zip_longest(lseq, rseq, fillvalue=None):
                if (
                    self.visit_dml_values(attrname, left_parent, ld, right_parent, rd, **kw)
                    is COMPARE_FAILED
                ):
                    return COMPARE_FAILED
