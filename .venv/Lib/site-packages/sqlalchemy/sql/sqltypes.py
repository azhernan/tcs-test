        ...
        if self.asdecimal:
            if dialect.supports_native_decimal:
                ...
                return processors.to_decimal_processor_factory(
                    decimal.Decimal,
                    (self.scale if self.scale is not None else self._default_decimal_return_scale),
                )
        else:
            if dialect.supports_native_decimal:
                return processors.to_float
...
class SchemaType(SchemaEventTarget, TypeEngineMixin):
    ...
    def _set_table(self, column, table):
        ...
        event.listen(
            table,
            "after_drop",
            util.portable_instancemethod(self._on_table_drop, {"variant_mapping": variant_mapping}),
        )
    ...
    def copy(self, **kw):
        return self.adapt(
            ...
            _create_events=True,
            metadata=(kw.get("_to_metadata", self.metadata) if self.metadata is not None else None),
        )
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
        ...
        if length_arg is not NO_ARG:
            if not _disable_warnings and length_arg is not None and length_arg < length:
                raise ValueError(
                    ...
                    " than the length of the longest enum value. %s < %s" % (length_arg, length)
                )
            length = length_arg
    ...
    def _setup_for_values(self, values, objects, kw):
        ...
        self._valid_lookup.update(
            [(value, self._valid_lookup[self._object_lookup[value]]) for value in values]
        )
...
class JSON(Indexable, TypeEngine[Any]):
    ...
    **Customizing the JSON Serializer**
    ...
    class Comparator(Indexable.Comparator[_T], Concatenable.Comparator[_T]):
        ...
        def _setup_getitem(self, index):
            if not isinstance(index, str) and isinstance(index, collections_abc.Sequence):
                ...
                index = coercions.expect(
                    ...
                    bindparam_type=(
                        JSON.JSONIntIndexType if isinstance(index, int) else JSON.JSONStrIndexType
                    ),
                )
                operator = operators.json_getitem_op
        ...
        def as_numeric(self, precision, scale, asdecimal=True):
            ...
            """
            return self._binary_w_type(Numeric(precision, scale, asdecimal=asdecimal), "as_numeric")
        ...
        def _binary_w_type(self, typ, method_name):
            if not isinstance(self.expr, elements.BinaryExpression) or self.expr.operator not in (
        ...
        if string_process:
            ...
            def process(value):
                if value is self.NULL:
                    value = None
                elif isinstance(value, elements.Null) or (value is None and self.none_as_null):
                    return None
...
class TupleType(TypeEngine[Tuple[Any, ...]]):
    ...
    def __init__(self, *types: _TypeEngineArgument[Any]):
        ...
        self.types = [
            item_type() if isinstance(item_type, type) else item_type for item_type in types
        ]
    ...
    def coerce_compared_value(self, op: Optional[OperatorType], value: Any) -> TypeEngine[Any]:
        if value is type_api._NO_VALUE_IN_LIST:
            ...
            return TupleType(
                *[typ.coerce_compared_value(op, elem) for typ, elem in zip(self.types, value)]
            )
    ...
    def result_processor(self, dialect, coltype):
        raise NotImplementedError(
            "The tuple type does not support being fetched " "as a column in a result row."
        )
    ...
    * When a new :class:`_schema.Column` is created,
    ...
    class Comparator(TypeEngine.Comparator[_T]):
        ...
        def _adapt_expression(
            ...
            if isinstance(other_comparator, NullType.Comparator) or not operators.is_commutative(
                op
            ):
...
class TableValueType(HasCacheKey, TypeEngine[Any]):
    ...
    def __init__(self, *elements: Union[str, _ColumnExpressionArgument[Any]]):
        self._elements = [coercions.expect(roles.StrAsPlainColumnRole, elem) for elem in elements]
...
def _resolve_value_to_type(value: Any) -> TypeEngine[Any]:
    ...
    if _result_type is False:
        ...
        if (
            ...
            raise exc.ArgumentError("Object %r is not legal as a SQL literal value" % (value,))
        return NULLTYPE
    else:
