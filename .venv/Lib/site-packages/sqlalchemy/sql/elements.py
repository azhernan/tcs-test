    ...
    def _make_proxy(
        ...
        **kw: Any,
        ...
        co: ColumnClause[_T] = ColumnClause(
            (coercions.expect(roles.TruncatedLabelRole, name) if name_is_truncatable else name),
    ...
    def __init__(
        ...
        if unique:
            self.key = _anonymous_label.safe_construct(
                id(self),
                (key if key is not None and not isinstance(key, _anonymous_label) else "param"),
                sanitize_key=True,
            )
...
class TextClause(
        ...
        **unique** bound parameters.  These are parameters that are
        ...
        for key, value in names_to_values.items():
            try:
                ...
                raise exc.ArgumentError(
                    "This text() construct doesn't define a " "bound parameter named %r" % key
                ) from err
            else:
    ...
    def self_group(self, against: Optional[OperatorType] = None) -> Union[Self, Grouping[Any]]:
    ...
    def __init__(
        ...
        *clauses: _ColumnExpressionArgument[Any],
        ...
        if self.group_contents:
            ...
            self.clauses = [
                coercions.expect(text_converter_role, clause, apply_propagate_attrs=self)
                for clause in clauses_iterator
            ]
...
class OperatorExpression(ColumnElement[_T]):
    ...
    def self_group(self, against: Optional[OperatorType] = None) -> Union[Self, Grouping[_T]]:
        if (
            self.group
...
class ExpressionClauseList(OperatorExpression[_T]):
    ...
    def __init__(
        ...
        *clauses: _ColumnExpressionArgument[Any],
        ...
        self.clauses = tuple(
            coercions.expect(roles.ExpressionElementRole, clause, apply_propagate_attrs=self)
...
class BooleanClauseList(ExpressionClauseList[bool]):
    ...
    def self_group(self, against: Optional[OperatorType] = None) -> Union[Self, Grouping[bool]]:
        if not self.clauses:
            return self
        else:
...
class UnaryExpression(ColumnElement[_T]):
    ...
    @classmethod
    def _create_desc(cls, column: _ColumnExpressionOrStrLabelArgument[_T]) -> UnaryExpression[_T]:
        return UnaryExpression(
            coercions.expect(roles.ByOfRole, column),
...
class _OverrideBinds(Grouping[_T]):
    ...
    def _gen_cache_key(
        ...
        bindparams.extend(
            (
                bp._with_value(self.translate[bp.key], maintain_key=True, required=False)
...
class WithinGroup(ColumnElement[_T]):
    ...
    def _from_objects(self) -> List[FromClause]:
        return list(
            itertools.chain(
                *[c._from_objects for c in (self.element, self.order_by) if c is not None]
            )
        )
...
class FunctionFilter(Generative, ColumnElement[_T]):
    ...
    def within_group_type(self, within_group: WithinGroup[_T]) -> Optional[TypeEngine[_T]]:
        return None
...
class Label(roles.LabeledColumnExprRole[_T], NamedColumn[_T]):
    ...
    def __init__(
        ...
        if name:
            ...
            self.name = _anonymous_label.safe_construct(id(self), getattr(element, "name", "anon"))
        ...
        self.type = type_api.to_instance(type_) if type_ is not None else self._element.type
