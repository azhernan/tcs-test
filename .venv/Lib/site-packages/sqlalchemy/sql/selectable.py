    ...
    def prefix_with(
        ...
        *prefixes: _TextCoercedExpressionArgument[Any],
        ...
        self._prefixes = self._prefixes + tuple(
            [(coercions.expect(roles.StatementOptionRole, p), dialect) for p in prefixes]
        )
        return self
    ...
    def suffix_with(
        ...
        *suffixes: _TextCoercedExpressionArgument[Any],
        ...
        self._suffixes = self._suffixes + tuple(
            [(coercions.expect(roles.StatementOptionRole, p), dialect) for p in suffixes]
        )
        return self
...
class Alias(roles.DMLTableRole, FromClauseAlias):
    ...
    def _factory(
        ...
        return coercions.expect(roles.FromClauseRole, selectable, allow_select=True).alias(
            name=name, flat=flat
        )
...
class Lateral(FromClauseAlias, LateralFromClause):
    ...
    def _factory(
        ...
        return coercions.expect(roles.FromClauseRole, selectable, explicit_subquery=True).lateral(
            name=name
        )
...
class TableSample(FromClauseAlias):
    ...
    _traverse_internals: _TraverseInternalsType = AliasedReturnsRows._traverse_internals + [
            ...
            if effective_name is not None:
                ...
                if effective_name in names:
                    ...
                    if hash(names[effective_name]) != hash(c):
                        ...
                        if table_qualified:
                            required_label_name = fallback_label_name = c._dedupe_anon_tq_label_idx(
                                dedupe_hash
                            )
                            dedupe_hash += 1
                        else:
                            required_label_name = fallback_label_name = c._dedupe_anon_label_idx(
                                dedupe_hash
                            )
                            dedupe_hash += 1
                        repeated = True
    ...
    def _offset_or_limit_clause_asint(
        ...
        try:
            ...
            raise exc.CompileError(
                "This SELECT structure does not use a simple " "integer value for %s" % attrname
            ) from err
        else:
...
class CompoundSelect(HasCompileState, GenerativeSelect, ExecutableReturnsRows):
    ...
    def __init__(
        ...
        *selects: _SelectStatementForCompoundArgument,
        ...
        self.selects = [
            coercions.expect(roles.CompoundElementRole, s, apply_propagate_attrs=self).self_group(
                against=self
            )
            for s in selects
        ]
    ...
    def _generate_fromclause_column_proxies(
        ...
        *,
        ...
        extra_col_iterator = zip(
            *[
                [c._annotate(dd) for c in stmt._all_selected_columns if is_column_element(c)]
                for dd, stmt in [({"weight": i + 1}, stmt) for i, stmt in enumerate(self.selects)]
            ]
        )
...
class SelectState(util.MemoizedSlots, CompileState):
    ...
    def _plugin_not_implemented(cls) -> NoReturn:
        raise NotImplementedError(
            "The default SELECT construct without plugins does not " "implement this method."
        )
    ...
    def get_column_descriptions(cls, statement: Select[Any]) -> List[Dict[str, Any]]:
        return [
            ...
            for _, name, _, element, _ in (statement._generate_columns_plus_names(False))
        ]
    ...
    def _normalize_froms(
        ...
        if froms:
            toremove = set(
                itertools.chain.from_iterable([_expand_cloned(f._hide_froms) for f in froms])
            )
            if toremove:
            ...
            if ambiguous_table_name_map is not None:
                ambiguous_table_name_map.update(
                    ...
                    if is_table(fr) and fr.schema and fr.name not in ambiguous_table_name_map
                )
    ...
    def _get_display_froms(
        ...
        if self.statement._auto_correlate and implicit_correlate_froms and len(froms) > 1:
            froms = [
                f for f in froms if f not in _cloned_intersection(froms, implicit_correlate_froms)
            ]
...
class Select(
    ...
    def filter_by(self, **kwargs: Any) -> Self:
        ...
        clauses = [
            _entity_namespace_key(from_entity, key) == value for key, value in kwargs.items()
        ]
    ...
    def add_columns(self, *entities: _ColumnsClauseArgument[Any]) -> Select[Any]:
        ...
        self._raw_columns = self._raw_columns + [
            coercions.expect(roles.ColumnsClauseRole, column, apply_propagate_attrs=self)
    ...
    def with_only_columns(
        ...
        **__kw: Any,
        ...
        if maintain_column_froms:
            self.select_from.non_generative(self, *self.columns_clause_froms)  # type: ignore
    ...
    def distinct(self, *expr: _ColumnExpressionArgument[Any]) -> Self:
        ...
        if expr:
            ...
            self._distinct_on = self._distinct_on + tuple(
                coercions.expect(roles.ByOfRole, e, apply_propagate_attrs=self) for e in expr
            )
        else:
            self._distinct = True
        return self
    ...
    def select_from(self, *froms: _FromClauseArgument) -> Self:
        ...
        self._from_obj += tuple(
            coercions.expect(roles.FromClauseRole, fromclause, apply_propagate_attrs=self)
        ...
        *fromclauses: Union[Literal[None, False], _FromClauseArgument],
        ...
        if not fromclauses or fromclauses[0] in {None, False}:
            if len(fromclauses) > 1:
                raise exc.ArgumentError(
                    "additional FROM objects not accepted when " "passing None/False to correlate()"
                )
            self._correlate = ()
        else:
