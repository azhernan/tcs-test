...
class ExpandedState(NamedTuple):
    ...
    def positional_parameters(self) -> Tuple[Any, ...]:
        ...
        if self.positiontup is None:
            raise exc.InvalidRequestError("statement does not use a positional paramstyle")
...
class SQLCompiler(Compiled):
    ...
    _positional_pattern = re.compile(f"{_pyformat_pattern.pattern}|{_post_compile_pattern.pattern}")
        ...
        **kwargs: Any,
        ...
        if self.isinsert or self.isupdate or self.isdelete:
            ...
            if self.isinsert or self.isupdate:
                ...
                if statement._inline:
                    ...
                    not self.isinsert
                    or (self.dialect.insert_executemany_returning and statement._return_defaults)
                ):
                    self.inline = True
        ...
        if self.state is CompilerState.STRING_APPLIED:
            ...
            if self._render_postcompile:
                ...
                self._process_parameters_for_postcompile(parameters, _populate_self=True)
    ...
    def effective_returning(self) -> Optional[Sequence[ColumnElement[Any]]]:
        ...
        if self.implicit_returning:
            ...
            return [c for c in self.statement._all_selected_columns if is_column_element(c)]
    ...
    def _process_numeric(self):
        ...
        if self._insertmanyvalues and self._values_bindparam is not None:
            ...
            order = itertools.chain(
                (name for name in self.bind_names.values() if name not in self._values_bindparam),
        ...
        if self.escaped_bind_names:
            ...
            param_pos = {
                self.escaped_bind_names.get(name, name): pos for name, pos in param_pos.items()
            }
        ...
        if self._insertmanyvalues:
            ...
            insert_crud_params = [
                (v[0], v[1], "%s", v[3]) for v in self._insertmanyvalues.insert_crud_params
            ]
    ...
    def construct_params(
        ...
        if params:
            ...
            for bindparam, name in self.bind_names.items():
                escaped_name = (
                    self.escaped_bind_names.get(name, name) if has_escaped_names else name
                )
    ...
    def _inserted_primary_key_from_returning_getter(self):
        ...
        def get(row, parameters):
            return row_fn(
                getter(row) if use_row else getter(parameters) for getter, use_row in getters
            )
    ...
    def visit_textual_label_reference(self, element, within_columns_clause=False, **kwargs):
        ...
        try:
            ...
            coercions._no_text_coercion(
                element.element,
                extra=("Can't resolve label reference for ORDER BY / " "GROUP BY / DISTINCT etc."),
        ...
        try:
            ...
            return self.process(col, within_columns_clause=within_columns_clause, **kwargs)
    ...
    def visit_textual_select(self, taf, compound_index=None, asfrom=False, **kw):
        ...
        populate_result_map = (
            toplevel
            or (compound_index == 0 and entry.get("need_result_map_for_compound", False))
    ...
    def _generate_delimited_list(self, elements, separator, **kw):
        return separator.join(s for s in (c._compiler_dispatch(self, **kw) for c in elements) if s)
            ...
            (
                ...
                if range_[0] is elements.RANGE_UNBOUNDED
                    ...
                    if range_[0] is elements.RANGE_CURRENT
                        ...
                        "%s PRECEDING" % (self.process(elements.literal(abs(range_[0])), **kw),)
                        if range_[0] < 0
            ...
            (
                ...
                if range_[1] is elements.RANGE_UNBOUNDED
                    ...
                    if range_[1] is elements.RANGE_CURRENT
                        ...
                        "%s PRECEDING" % (self.process(elements.literal(abs(range_[1])), **kw),)
                        if range_[1] < 0
    ...
    def visit_over(self, over, **kwargs):
        ...
        if over.range_:
            range_ = "RANGE BETWEEN %s" % self._format_frame_clause(over.range_, **kwargs)
        elif over.rows:
    ...
    def visit_unary(self, unary, add_to_result_map=None, result_map_targets=(), **kw):
        ...
        if unary.operator:
            if unary.modifier:
                raise exc.CompileError(
                    "Unary expression does not support operator " "and modifier simultaneously"
                )
            disp = self._get_operator_dispatch(unary.operator, "unary", "operator")
            if disp:
                return disp(unary, unary.operator, **kw)
            else:
                return self._generate_generic_unary_operator(unary, OPERATORS[unary.operator], **kw)
        elif unary.modifier:
            disp = self._get_operator_dispatch(unary.modifier, "unary", "modifier")
            if disp:
                return disp(unary, unary.modifier, **kw)
            else:
                return self._generate_generic_unary_modifier(unary, OPERATORS[unary.modifier], **kw)
        else:
    ...
    def visit_floordiv_binary(self, binary, operator, **kw):
        if self.dialect.div_is_floordiv and binary.right.type._type_affinity is sqltypes.Integer:
            return self.process(binary.left, **kw) + " / " + self.process(binary.right, **kw)
        else:
            return "FLOOR(%s)" % (
    ...
    def visit_not_match_op_binary(self, binary, operator, **kw):
        return "NOT %s" % self.visit_binary(binary, override_operator=operators.match_op)
    ...
    def _literal_execute_expanding_parameter_literal_binds(
        ...
        if not values:
            ...
            if typ_dialect_impl._is_tuple_type:
                ...
                ) + self.visit_empty_set_op_expr(parameter.type.types, parameter.expand_op)
            ...
            replacement_expression = (
                ...
                % (
                    ", ".join(
                        ...
                        for value, param_type in zip(tuple_element, parameter.type.types)
                    )
                )
    ...
    def _literal_execute_expanding_parameter(self, name, parameter, values):
        if parameter.literal_execute:
            return self._literal_execute_expanding_parameter_literal_binds(parameter, values)
        ...
        if not values:
            ...
            replacement_expression = ("VALUES " if dialect.tuple_in_values else "") + ", ".join(
                "(%s)"
                % (
                    ", ".join(
            ...
            to_update = [("%s_%s" % (name, i), value) for i, value in enumerate(values, 1)]
            replacement_expression = ", ".join(
    ...
    def visit_icontains_op_binary(self, binary, operator, **kw):
        ...
        binary.right = percent.concat(ilike_case_insensitive(binary.right)).concat(percent)
    ...
    def visit_regexp_replace_op_binary(self, binary, operator, **kw):
        raise exc.CompileError(
            "%s dialect does not support regular expression replacements" % self.dialect.name
        )
    ...
    def visit_bindparam(
        ...
        **kwargs,
        ...
        if literal_binds:
            ret = self.render_literal_bindparam(bindparam, within_columns_clause=True, **kwargs)
            if bindparam.expanding:
                ret = "(%s)" % ret
            return ret
    ...
    def bindparam_string(
        ...
        **kw: Any,
        ...
        if escaped_from:
            self.escaped_bind_names = self.escaped_bind_names.union({escaped_from: name})
        if post_compile:
        ...
        **kwargs: Any,
        ...
        if existing_cte is not None:
            ...
            if cte is existing_cte._restates or cte is existing_cte:
                ...
                if (
                    cte._is_clone_of is not None or existing_cte._is_clone_of is not None
                ) and cte.compare(existing_cte):
                    is_new_cte = False
                else:
                    raise exc.CompileError(
                        "Multiple, unrelated CTEs found with " "the same name: %r" % cte_name
                    )
        ...
        if is_new_cte:
            ...
            if not cte_pre_alias_name and cte not in self_ctes:
                ...
                if cte.recursive:
                    ...
                    text += "(%s)" % (
                        ", ".join(
                            self.preparer.format_label_name(ident, anon_map=self.anon_map)
    ...
    def visit_alias(
        ...
        **kwargs,
        ...
        if ashint:
            ...
            inner = alias.element._compiler_dispatch(self, asfrom=True, lateral=lateral, **kwargs)
            if subquery:
                inner = "(%s)" % (inner,)
                ...
                ret += "(%s)" % (
                    ", ".join(
                        ...
                        % (
                            ...
                            (
                                " %s"
                                % self.dialect.type_compiler_instance.process(col.type, **kwargs)
    ...
    def visit_tablesample(self, tablesample, asfrom=False, **kw):
        ...
        if tablesample.seed is not None:
            text += " REPEATABLE (%s)" % (tablesample.seed._compiler_dispatch(self, **kw))
        ...
        if asfrom:
            ...
            if name:
                ...
                v = "%s(%s)%s (%s)" % (
                    ...
                    (", ".join(c._compiler_dispatch(self, **kw) for c in element.columns)),
                )
            else:
                v = "%s(%s)" % (lateral, v)
        return v
    ...
    **kw)`` and returns a select object.   this is used for structural changes
    ...
    def visit_select(
        ...
        **kwargs,
        ...
        if populate_result_map and select_wraps_for is not None:
            ...
            self._result_columns = [
                ResultColumnsEntry(key, name, tuple(translate.get(o, o) for o in obj), type_)
    ...
    def _render_cte_clause(
        ...
        if nesting_level and nesting_level > 1:
            ...
            for cte in list(self.ctes.keys()):
                cte_level, cte_name, cte_opts = self.level_name_by_cte[cte._get_reference_cte()]
                nesting = cte.nesting or cte_opts.nesting
    ...
    def fetch_clause(
        ...
        **kw,
        ...
        if select._offset_clause is not None:
            ...
            if use_literal_execute_for_simple_int and select._simple_int_clause(offset_clause):
        ...
        if fetch_clause is not None:
            if use_literal_execute_for_simple_int and select._simple_int_clause(fetch_clause):
    ...
    _sentinel_col_non_autoinc_lookup = util.immutabledict(
        {
            ...
            _SentinelDefaultCharacterization.IDENTITY: (InsertmanyvaluesSentinelOpts.IDENTITY),
            _SentinelDefaultCharacterization.SEQUENCE: (InsertmanyvaluesSentinelOpts.SEQUENCE),
        }
    )
    _sentinel_col_autoinc_lookup = _sentinel_col_non_autoinc_lookup.union(
        {
            _SentinelDefaultCharacterization.NONE: (InsertmanyvaluesSentinelOpts.AUTOINCREMENT),
        }
    )
    ...
    def _deliver_insertmanyvalues_batches(
        ...
        if use_row_at_a_time:
            for batchnum, (param, compiled_param) in enumerate(
                ...
                yield _InsertManyValuesBatch(
                    ...
                    [param],
                    ([_sentinel_from_params(compiled_param)] if _sentinel_from_params else []),
                    1,
                    batchnum,
                    lenparams,
                    sort_by_parameter_order,
        ...
        if not self.positional:
            ...
            if imv.embed_values_counter:
                executemany_values_w_comma = f"({imv_single_values_expr}, _IMV_VALUES_COUNTER), "
            else:
            ...
            if all_names_we_will_expand:
                ...
                all_expand_positions = {
                    idx for idx, name in enumerate(positiontup) if name in all_names_we_will_expand
                }
                expand_pos_lower_index = min(all_expand_positions)
                expand_pos_upper_index = max(all_expand_positions) + 1
                assert len(all_expand_positions) == expand_pos_upper_index - expand_pos_lower_index
            ...
            if self.positional:
                ...
                if imv.embed_values_counter:
                    expanded_values_string = (
                        "".join(
                            executemany_values_w_comma.replace("_IMV_VALUES_COUNTER", str(i))
                            for i, _ in enumerate(batch)
                        )
                    )[:-2]
                else:
                    expanded_values_string = ((executemany_values_w_comma * current_batch_size))[
                        :-2
                    ]
                ...
                if self._numeric_binds and num_ins_params > 0:
                    ...
                    positions = tuple(
                        f"{self._numeric_binds_identifier_char}{i}" for i in range(start, end)
                    )
                    expanded_values_string = expanded_values_string % positions

                replaced_statement = statement.replace("__EXECMANY_TOKEN__", expanded_values_string)

                replaced_parameters = tuple(itertools.chain.from_iterable(batch_iterator))

                replaced_parameters = extra_params_left + replaced_parameters + extra_params_right
        ...
        if insert_stmt.select is not None:
            ...
            if use_insertmanyvalues:
                self._insertmanyvalues = _InsertManyValues(
                    ...
                    num_sentinel_columns=(len(add_sentinel_cols) if add_sentinel_cols else 0),
    ...
    def visit_update(self, update_stmt, visiting_cte=None, **kw):
        ...
        text += ", ".join(
            expr + "=" + value
            for _, expr, value, _ in cast("List[Tuple[Any, str, str, Any]]", crud_params)
        )
...
class DDLCompiler(Compiled):
    ...
    def visit_set_column_comment(self, create, **kw):
        return "COMMENT ON COLUMN %s IS %s" % (
            self.preparer.format_column(create.element, use_table=True, use_schema=True),
            self.sql_compiler.render_literal_value(create.element.comment, sqltypes.String()),
        )
    ...
    def visit_drop_constraint(self, drop, **kw):
        ...
        if formatted_name is None:
            raise exc.CompileError(
                "Can't emit DROP CONSTRAINT for constraint %r; " "it has no name" % drop.element
            )
...
class IdentifierPreparer:
    ...
    def validate_sql_phrase(self, element, reg):
        ...
        if element is not None and not reg.match(element):
            raise exc.CompileError(
                "Unexpected SQL phrase: %r (matching against %r)" % (element, reg.pattern)
            )
        return element
    ...
    def format_column(
        ...
        if not getattr(column, "is_literal", False):
            if use_table:
                return (
                    self.format_table(column.table, use_schema=use_schema, name=table_name)
                    + "."
                    + self.quote(name)
                )
            else:
    ...
    def _r_identifiers(self):
        ...
        r = re.compile(
            ...
            r"|([^\.]+))(?=\.|$))+" % {"initial": initial, "final": final, "escaped": escaped_final}
        )
        return r
