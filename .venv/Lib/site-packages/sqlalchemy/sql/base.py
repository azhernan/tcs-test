...
class _DefaultDescriptionTuple(NamedTuple):
    ...
    @classmethod
    def _from_column_default(cls, default: Optional[DefaultGenerator]) -> _DefaultDescriptionTuple:
        return (
            ...
            if default and (default.has_arg or (not default.for_update and default.is_sentinel))
...
def _from_objects(
    *elements: Union[ColumnElement[Any], FromClause, TextClause, _JoinTargetElement]
    ...
    return itertools.chain.from_iterable([element._from_objects for element in elements])
...
class DialectKWArgs:
    ...
    def _validate_dialect_kwargs(self, kwargs: Dict[str, Any]) -> None:
        ...
        for k in kwargs:
            ...
            if not m:
                raise TypeError(
                    "Additional arguments should be " "named <dialectname>_<argument>, got '%s'" % k
                )
...
class Generative(HasMemoized):
    ...
    def _generate(self) -> Self:
        ...
        if skip:
            ...
            s.__dict__ = {k: v for k, v in self.__dict__.copy().items() if k not in skip}
        else:
    ...
    def __add__(self, other):
        ...
        if set(other).difference(self._cache_attrs):
            raise TypeError(
                ...
                "Options class %s: %r" % (self, set(other).difference(self._cache_attrs))
            )
...
class Options(metaclass=_MetaOptions):
    ...
    def __init_subclass__(cls) -> None:
        ...
        cls._cache_attrs = tuple(
            sorted(
                d for d in dict_ if not d.startswith("__") and d not in ("_cache_key_traversal",)
            )
        )
    ...
    def __repr__(self):
        ...
        return "%s(%s)" % (
            ...
            ", ".join(
                "%s=%r" % (k, self.__dict__[k]) for k in self._cache_attrs if k in self.__dict__
            ),
        )
...
class ColumnCollection(Generic[_COLKEY, _COL_co]):
    ...
    def get(self, key: str, default: Optional[_COL] = None) -> Optional[Union[_COL_co, _COL]]:
...
class DedupeColumnCollection(ColumnCollection[str, _NAMEDCOL]):
    ...
    def add(self, column: _NAMEDCOL, key: Optional[str] = None) -> None:  # type: ignore[override]
        if key is not None and column.key != key:
            raise exc.ArgumentError(
                "DedupeColumnCollection requires columns be under " "the same key as their .key"
            )
        key = column.key
