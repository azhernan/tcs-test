...
class AsyncSession(ReversibleProxy[Session]):
    ...
    def __init__(
        ...
        **kw: Any,
        ...
        if binds:
            self.binds = binds
            sync_binds = {key: engine._get_sync_engine_or_connection(b) for key, b in binds.items()}
    ...
    async def run_sync(
        ...
        **kw: _P.kwargs,
        ...
        return await greenlet_spawn(fn, self.sync_session, *arg, _require_await=False, **kw)
    ...
    async def execute(
        ...
        **kw: Any,
        ...
        if execution_options:
            execution_options = util.immutabledict(execution_options).union(_EXECUTE_OPTIONS)
        else:
    ...
    async def stream(
        ...
        **kw: Any,
        ...
        if execution_options:
            execution_options = util.immutabledict(execution_options).union(_STREAM_OPTIONS)
        else:
    ...
    async def merge(
        ...
        *,
        ...
        """
        return await greenlet_spawn(self.sync_session.merge, instance, load=load, options=options)
