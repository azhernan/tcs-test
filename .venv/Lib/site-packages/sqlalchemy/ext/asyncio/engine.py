...
def create_async_engine(url: Union[str, URL], **kw: Any) -> AsyncEngine:
    ...
    if async_creator:
        ...
        def creator() -> Any:
            ...
            return sync_engine.dialect.dbapi.connect(async_creator_fn=async_creator)  # type: ignore
...
def async_engine_from_config(
    ...
    """
    options = {
        key[len(prefix) :]: value for key, value in configuration.items() if key.startswith(prefix)
    }
...
class AsyncConnection(
    ...
    def _regenerate_proxy_for_target(cls, target: Connection) -> AsyncConnection:
        return AsyncConnection(AsyncEngine._retrieve_proxy_for_target(target.engine), target)
