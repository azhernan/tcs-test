...
class AssociationProxyInstance(SQLORMOperations[_T]):
    ...
    def for_proxy(
        ...
        if not isinstance(prop, orm.RelationshipProperty):
            raise NotImplementedError(
                "association proxy to a non-relationship " "intermediary is not supported"
            ) from None
    ...
    def _comparator(self) -> PropComparator[Any]:
        return getattr(self.owning_class, self.target_collection).comparator  # type: ignore
    ...
    def _new(self, lazy_collection: _LazyCollectionProtocol[_T]) -> Tuple[Type[Any], _T]:
        ...
        if self.parent.proxy_factory:
            return (
                collection_class,
                self.parent.proxy_factory(lazy_collection, creator, self.value_attr, self),
            )
    ...
    def _set(self, proxy: _AssociationCollection[Any], values: Iterable[Any]) -> None:
        if self.parent.proxy_bulk_set:
            ...
            raise exc.ArgumentError(
                "no proxy_bulk_set supplied for custom " "collection_class implementation"
            )

    def _inflate(self, proxy: _AssociationCollection[Any]) -> None:
        creator = (
            self.parent.creator and self.parent.creator or cast(_CreatorProtocol, self.target_class)
        )
    ...
    def _criterion_exists(
        ...
        **kwargs: Any,
        ...
        if self._target_is_object:
            ...
            if kwargs:
                raise exc.ArgumentError(
                    "Can't apply keyword arguments to column-targeted " "association proxy; use =="
                )
            elif is_has and criterion is not None:
                raise exc.ArgumentError(
                    "Non-empty has() not allowed for " "column-targeted association proxy; use =="
                )
    ...
    def contains(self, other: Any, **kw: Any) -> ColumnElement[bool]:
        ...
        if target_assoc is not None:
            return self._comparator._criterion_exists(
                target_assoc.contains(other) if not target_assoc.scalar else target_assoc == other
            )
...
class _AssociationSet(_AssociationSingleItem[_T], MutableSet[_T]):
    ...
    def __ior__(self, other: AbstractSet[_S]) -> MutableSet[Union[_T, _S]]:  # type: ignore
