...
def get_mapped_attributes(
    ...
    mapped_attributes: Optional[List[JsonDict]] = _get_info_metadata(info, "mapped_attributes")
...
def get_callexpr_kwarg(
    ...
    *,
    ...
    if isinstance(kwarg, expr_types if expr_types is not None else (NameExpr, CallExpr)):
        return kwarg
...
def unbound_to_instance(api: SemanticAnalyzerPluginInterface, typ: Type) -> Type:
    ...
    if typ.name == "Optional":
        ...
        return unbound_to_instance(
            api,
            UnionType([unbound_to_instance(api, typ_arg) for typ_arg in typ.args] + [NoneType()]),
        )
...
def serialize_type(typ: Type) -> Union[str, JsonDict]:
    ...
    if hasattr(typ, "args"):
        typ.args = tuple(
            (a.resolve_string_annotation() if hasattr(a, "resolve_string_annotation") else a)
            for a in typ.args
        )
