...
class Pool(log.Identified, event.EventTarget):
    ...
    def _close_connection(self, connection: DBAPIConnection, *, terminate: bool = False) -> None:
        self.logger.debug(
            "%s connection %r",
        ...
        try:
            ...
            self.logger.error(
                f"Exception {'terminating' if terminate else 'closing'} " f"connection %r",
                connection,
                exc_info=True,
            )
...
class _ConnectionRecord(ConnectionPoolEntry):
    ...
    def get_connection(self) -> DBAPIConnection:
        ...
        if self.dbapi_connection is None:
            ...
            self.__pool.logger.info(
                "Connection %r invalidated due to local soft invalidation; " + "recycling",
                self.dbapi_connection,
            )
            recycle = True
    ...
    def __connect(self) -> None:
        ...
        try:
            ...
            if pool.dispatch.first_connect:
                pool.dispatch.first_connect.for_modify(pool.dispatch).exec_once_unless_exception(
...
_strong_ref_connection_records: Dict[weakref.ref[_ConnectionFairy], _ConnectionRecord] = {}
