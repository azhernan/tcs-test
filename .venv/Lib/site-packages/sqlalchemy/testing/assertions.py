...
def expect_deprecated(*messages, **kw):
    return _expect_warnings_sqla_only(sa_exc.SADeprecationWarning, messages, **kw)


def expect_deprecated_20(*messages, **kw):
    return _expect_warnings_sqla_only(sa_exc.Base20DeprecationWarning, messages, **kw)
    ...
    def assert_compile(
        ...
        if checkpositional is not None:
            if render_postcompile:
                expanded_state = c.construct_expanded_state(params, escape_names=False)
                eq_(
                    tuple([expanded_state.parameters[x] for x in expanded_state.positiontup]),
                    checkpositional,
                )
            else:
...
class AssertsExecutionResults:
    ...
    def assert_list(self, result, class_, list_):
        self.assert_(
            ...
            "result list is not the same size as test list, " + "for class " + class_.__name__,
        )
        ...
        for key, value in desc.items():
            if isinstance(value, tuple):
                ...
                self.assert_(
                    ...
                    "attribute %s value %s does not match %s" % (key, getattr(rowobj, key), value),
                )
