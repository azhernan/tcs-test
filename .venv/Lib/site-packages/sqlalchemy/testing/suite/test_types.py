...
class IntegerTest(_LiteralRoundTripFixture, fixtures.TestBase):
    ...
    def test_huge_int_auto_accommodation(self, connection, intvalue):
        ...
        eq_(
            connection.scalar(select(intvalue).where(literal(intvalue) == intvalue)),
            intvalue,
        )
...
class TrueDivTest(fixtures.TestBase):
    ...
    def test_truediv_integer(self, connection, left, right, expected):
        ...
        eq_(
            connection.scalar(
                select(
                    literal_column(left, type_=Integer()) / literal_column(right, type_=Integer())
                )
            ),
            expected,
        )
    ...
    def test_truediv_float(self, connection, left, right, expected):
        ...
        eq_(
            connection.scalar(
                select(literal_column(left, type_=Float()) / literal_column(right, type_=Float()))
            ),
            expected,
        )
    ...
    def test_floordiv_numeric(self, connection, left, right, expected):
        ...
        eq_(
            connection.scalar(
                select(
                    literal_column(left, type_=Numeric()) // literal_column(right, type_=Numeric())
                )
            ),
            decimal.Decimal(expected),
        )
...
class BooleanTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    def test_whereclause(self):
        ...
        with config.db.begin() as conn:
            ...
            eq_(
                conn.scalar(select(boolean_table.c.id).where(boolean_table.c.unconstrained_value)),
                1,
            )
            eq_(
                conn.scalar(select(boolean_table.c.id).where(~boolean_table.c.value)),
...
class JSONTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    def test_round_trip_json_null_as_json_null(self, connection):
        ...
        eq_(
            conn.scalar(select(self.tables.data_table.c.name).where(cast(col, String) == "null")),
            "r1",
        )
...
class EnumTest(_LiteralRoundTripFixture, fixtures.TablesTest):
    ...
    def test_round_trip(self, data, connection):
        connection.execute(self.tables.enum_table.insert(), {"id": 1, "enum_data": data})

        eq_(
            connection.scalar(
                select(self.tables.enum_table.c.enum_data).where(self.tables.enum_table.c.id == 1)
            ),
            data,
        )
    ...
    def test_round_trip_executemany(self, connection):
        ...
        eq_(
            connection.scalars(
                select(self.tables.enum_table.c.enum_data).order_by(self.tables.enum_table.c.id)
            ).all(),
    ...
    def test_round_trip_executemany_returning(self, connection):
        result = connection.execute(
            self.tables.enum_table.insert().returning(self.tables.enum_table.c.enum_data),
            [
                {"id": 1, "enum_data": "b%percent"},
                {"id": 2, "enum_data": "réveillé"},
                {"id": 3, "enum_data": "b"},
