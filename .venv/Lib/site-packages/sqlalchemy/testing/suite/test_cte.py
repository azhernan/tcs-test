...
class CTETest(fixtures.TablesTest):
    ...
    def test_insert_from_select_round_trip(self, connection):
        ...
        cte = select(some_table).where(some_table.c.data.in_(["d2", "d3", "d4"])).cte("some_cte")
        connection.execute(
            some_other_table.insert().from_select(["id", "data", "parent_id"], select(cte))
        )
        eq_(
            connection.execute(select(some_other_table).order_by(some_other_table.c.id)).fetchall(),
    ...
    def test_update_from_round_trip(self, connection):
        ...
        connection.execute(
            some_other_table.insert().from_select(["id", "data", "parent_id"], select(some_table))
        )
    ...
    def test_delete_from_round_trip(self, connection):
        ...
        cte = select(some_table).where(some_table.c.data.in_(["d2", "d3", "d4"])).cte("some_cte")
        connection.execute(some_other_table.delete().where(some_other_table.c.data == cte.c.data))
        eq_(
            connection.execute(select(some_other_table).order_by(some_other_table.c.id)).fetchall(),
            [(1, "d1", None), (5, "d5", 3)],
        )
    ...
    def test_delete_scalar_subq_round_trip(self, connection):
        ...
        connection.execute(
            some_other_table.delete().where(
                ...
                == select(cte.c.data).where(cte.c.id == some_other_table.c.id).scalar_subquery()
            )
        )
        eq_(
            connection.execute(select(some_other_table).order_by(some_other_table.c.id)).fetchall(),
