...
class ValuesExpressionTest(fixtures.TestBase):
    ...
    def test_tuples(self, connection):
        value_expr = values(column("id", Integer), column("name", String), name="my_values").data(
...
class FetchLimitOffsetTest(fixtures.TablesTest):
    ...
    def test_bound_limit_offset(self, connection):
        ...
        self._assert_result(
            connection,
            select(table).order_by(table.c.id).limit(bindparam("l")).offset(bindparam("o")),
            [(2, 2, 3), (3, 3, 4)],
            params={"l": 2, "o": 1},
        )
    ...
    def test_bound_fetch_offset(self, connection):
        ...
        self._assert_result(
            connection,
            select(table).order_by(table.c.id).fetch(bindparam("f")).offset(bindparam("o")),
            [(2, 2, 3), (3, 3, 4)],
            params={"f": 2, "o": 1},
        )
    ...
    def test_expr_offset(self, connection):
        ...
        self._assert_result(
            connection,
            select(table).order_by(table.c.id).offset(literal_column("1") + literal_column("2")),
            [(4, 4, 5), (5, 4, 6)],
        )
    ...
    def test_expr_limit(self, connection):
        ...
        self._assert_result(
            connection,
            select(table).order_by(table.c.id).limit(literal_column("1") + literal_column("2")),
            [(1, 1, 2), (2, 2, 3), (3, 3, 4)],
        )
    ...
    def test_simple_fetch_percent_ties(self, connection):
        ...
        self._assert_result(
            connection,
            select(table).order_by(table.c.x.desc()).fetch(20, percent=True, with_ties=True),
            [(4, 4, 5), (5, 4, 6)],
            set_=True,
        )
    ...
    def test_fetch_offset_percent_ties(self, connection):
        table = self.tables.some_table
        fa = connection.execute(
            select(table).order_by(table.c.x).fetch(40, percent=True, with_ties=True).offset(2)
        ).fetchall()
        eq_(fa[0], (3, 3, 4))
        eq_(set(fa), {(3, 3, 4), (4, 4, 5), (5, 4, 6)})
...
class PostCompileParamsTest(AssertsExecutionResults, AssertsCompiledSQL, fixtures.TablesTest):
    ...
    def test_compile(self):
        ...
        self.assert_compile(
            stmt,
            "SELECT some_table.id FROM some_table " "WHERE some_table.x = __[POSTCOMPILE_q]",
            {},
        )
    ...
    def test_execute_tuple_expanding_plus_literal_execute(self):
        ...
        stmt = select(table.c.id).where(
            tuple_(table.c.x, table.c.y).in_(bindparam("q", expanding=True, literal_execute=True))
        )
    ...
    def test_execute_tuple_expanding_plus_literal_heterogeneous_execute(self):
        ...
        stmt = select(table.c.id).where(
            tuple_(table.c.x, table.c.z).in_(bindparam("q", expanding=True, literal_execute=True))
        )
...
class ExpandingBoundInTest(fixtures.TablesTest):
    ...
    def test_empty_heterogeneous_tuples_direct(self):
        ...
        def go(val, expected):
            stmt = (
                select(table.c.id).where(tuple_(table.c.x, table.c.z).in_(val)).order_by(table.c.id)
            )
    ...
    def test_empty_homogeneous_tuples_direct(self):
        ...
        def go(val, expected):
            stmt = (
                select(table.c.id).where(tuple_(table.c.x, table.c.y).in_(val)).order_by(table.c.id)
            )
            self._assert_result(stmt, expected)
    ...
    def test_bound_in_heterogeneous_two_tuple_text_bindparam(self):
        ...
        stmt = text("select id FROM some_table WHERE (x, z) IN :q ORDER BY id").bindparams(
            bindparam("q", expanding=True)
        )
        self._assert_result(
            stmt,
...
class ComputedColumnTest(fixtures.TablesTest):
    ...
    def test_select_all(self):
        with config.db.connect() as conn:
            res = conn.execute(
                select(text("*")).select_from(self.tables.square).order_by(self.tables.square.c.id)
            ).fetchall()
...
class IdentityColumnTest(fixtures.TablesTest):
    ...
    def test_select_all(self, connection):
        res = connection.execute(
            select(text("*")).select_from(self.tables.tbl_a).order_by(self.tables.tbl_a.c.id)
        ).fetchall()
        eq_(res, [(42, "a"), (43, "b")])

        res = connection.execute(
            select(text("*")).select_from(self.tables.tbl_b).order_by(self.tables.tbl_b.c.id)
        ).fetchall()
...
class WindowFunctionTest(fixtures.TablesTest):
    ...
    def test_window(self, connection):
        ...
        rows = connection.execute(
            select(func.max(some_table.c.col2).over(order_by=[some_table.c.col1.desc()])).where(
                some_table.c.col1 < 20
            )
        ).all()
