...
class InsertBehaviorTest(fixtures.TablesTest):
    ...
    def test_insert_from_select_autoinc(self, connection):
        ...
        result = connection.execute(
            dest_table.insert().from_select(
                ("data",),
                select(src_table.c.data).where(src_table.c.data.in_(["data2", "data3"])),
            )
        )
    ...
    def test_insert_from_select(self, connection):
        ...
        connection.execute(
            ...
            .from_select(
                ("id", "data"),
                select(table.c.id + 5, table.c.data).where(table.c.data.in_(["data2", "data3"])),
            )
        )
    ...
    def test_insert_from_select_with_defaults(self, connection):
        ...
        eq_(
            connection.execute(select(table).order_by(table.c.data, table.c.id)).fetchall(),
            [
                (1, "data1", 5, 4),
                (2, "data2", 5, 4),
                (7, "data2", 5, 4),
...
class ReturningTest(fixtures.TablesTest):
    ...
    def test_insertmanyvalues_returning(self, connection):
        r = connection.execute(
            self.tables.autoinc_pk.insert().returning(self.tables.autoinc_pk.c.id),
            [
                {"data": "d1"},
                {"data": "d2"},
                {"data": "d3"},
    ...
    def test_insert_w_floats(
        ...
        result = connection.execute(
            ...
            ([{"value": value} for i in range(10)] if multiple_rows else {"value": value}),
        )
        ...
        if do_rounding:
            ...
            eq_(
                {round(val_, 5) for val_ in connection.scalars(select(t.c.value))},
                {round(value, 5)},
            )
        else:
            eq_(
