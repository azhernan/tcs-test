...
def pytest_configure(config: pytest.Config):
    ...
    if plugin_base.exclude_tags or plugin_base.include_tags:
        new_expr = " and ".join(
            list(plugin_base.include_tags) + [f"not {tag}" for tag in plugin_base.exclude_tags]
        )
...
def _parametrize_cls(module, cls):
    ...
    for full_param_set in itertools.product(*[params for argname, params in _sa_parametrize]):
        ...
        parametrized_name = "_".join(
            re.sub(r"\W", "", token) for param in full_param_set for token in param.id.split("-")
        )
...
class PytestFixtureFunctions(plugin_base.FixtureFunctions):
    ...
    _combination_id_fns = {
        ...
        "r": repr,
        "s": str,
        "n": lambda obj: (obj.__name__ if hasattr(obj, "__name__") else type(obj).__name__),
    }
