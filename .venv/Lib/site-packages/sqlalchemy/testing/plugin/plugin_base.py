    ...
    if os.environ.get("REQUIRE_SQLALCHEMY_CEXT", "0") == "1":
        ...
        try:
            ...
            raise AssertionError(
                "REQUIRE_SQLALCHEMY_CEXT is set but can't import the " "cython extensions"
            ) from err
        ...
        for db_token in options.db:
            for db in re.split(r"[,\s]+", db_token):
                if db not in file_config.options("db"):
                    raise RuntimeError(
                        "Unknown URI specifier '%s'.  " "Specify --dbs for known uris." % db
                    )
                else:
    ...
    if not all_configs:
        msg = "'%s.%s' unsupported on any DB implementation %s%s" % (
            ...
            ", ".join(
                ...
                % (
                    config_obj.db.name,
                    ".".join(str(dig) for dig in exclusions._server_version(config_obj.db)),
