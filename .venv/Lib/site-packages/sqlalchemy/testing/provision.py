...
def generate_db_urls(db_urls, extra_drivers):
    ...
    for url_obj, dialect in urls_plus_dialects:
        ...
        if backend not in backend_to_driver_we_need:
            backend_to_driver_we_need[backend] = extra_per_backend = set(extra_drivers).difference(
                backend_to_driver_we_already_have[backend]
            )
        else:
            ...
            if config.requirements.schemas.enabled_for_config(cfg):
                for schema_name in [cfg.test_schema, cfg.test_schema_2]:
                    for seq in inspector.get_sequence_names(schema=schema_name):
                        conn.execute(ddl.DropSequence(schema.Sequence(seq, schema=schema_name)))
...
def upsert(cfg, table, returning, *, set_lambda=None, sort_by_parameter_order=False):
    ...
    """
    raise NotImplementedError(f"backend does not include an upsert implementation: {cfg.db.url}")
