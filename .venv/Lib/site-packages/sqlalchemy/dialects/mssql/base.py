...
class MSExecutionContext(default.DefaultExecutionContext):
    ...
    def pre_exec(self):
        ...
        if self.isinsert:
            ...
            if id_column is not None and (not isinstance(id_column.default, Sequence)):
                ...
                self._enable_identity_insert = (id_column.key in self.compiled_parameters[0]) or (
            ...
            if self._enable_identity_insert:
                self.root_connection._cursor_execute(
                    self.cursor,
                    self._opt_encode(
                        "SET IDENTITY_INSERT %s ON" % self.identifier_preparer.format_table(tbl)
                    ),
                    (),
                    self,
                )
        ...
        if self._enable_identity_insert:
            ...
            conn._cursor_execute(
                ...
                self._opt_encode(
                    ...
                    % self.identifier_preparer.format_table(self.compiled.compile_state.dml_table)
                ),
                (),
                self,
            )
    ...
    def fire_sequence(self, seq, type_):
        return self._execute_scalar(
            ("SELECT NEXT VALUE FOR %s" % self.identifier_preparer.format_sequence(seq)),
            type_,
        )
...
class MSSQLCompiler(compiler.SQLCompiler):
    ...
    def _check_can_use_fetch_limit(self, select):
        ...
        if select._fetch_clause_options is not None and (
            select._fetch_clause_options["percent"] or select._fetch_clause_options["with_ties"]
        ):
            raise exc.CompileError(
    ...
    def translate_select_structure(self, select_stmt, **kwargs):
        ...
        if (
            ...
            _order_by_clauses = [
                sql_util.unwrap_label_reference(elem) for elem in select._order_by_clause.clauses
            ]
            ...
            select = (
                select.add_columns(
                    sql.func.ROW_NUMBER().over(order_by=_order_by_clauses).label("mssql_rn")
                )
                .order_by(None)
                .alias()
            )
...
class MSDDLCompiler(compiler.DDLCompiler):
    def get_column_specification(self, column, **kwargs):
        ...
        elif (column is column.table._autoincrement_column or column.autoincrement is True) and (
    ...
    def visit_set_table_comment(self, create, **kw):
        ...
        return (
            ...
            "{}, 'schema', {}, 'table', {}".format(
                self.sql_compiler.render_literal_value(create.element.comment, sqltypes.NVARCHAR()),
...
class MSDialect(default.DefaultDialect):
    ...
    def do_rollback(self, dbapi_connection):
        try:
            ...
            if self.ignore_no_transaction_on_rollback and re.match(r".*\b111214\b", str(e)):
                util.warn(
        ...
        s = (
            ...
            .outerjoin(
                ...
                onclause=sql.and_(
                    ...
                    ischema.extended_properties.c.minor_id == columns.c.ordinal_position,
