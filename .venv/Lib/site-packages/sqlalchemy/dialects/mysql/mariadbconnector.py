...
class MySQLDialect_mariadbconnector(MySQLDialect):
        ...
        if self.dbapi is not None:
            if self._dbapi_version < mariadb_cpy_minimum_version:
                raise NotImplementedError(
                    ...
                    "Connector/Python is %s" % ".".join(str(x) for x in mariadb_cpy_minimum_version)
                )
    ...
    def do_begin_twophase(self, connection, xid):
        connection.execute(
            sql.text("XA BEGIN :xid").bindparams(sql.bindparam("xid", xid, literal_execute=True))
        )

    def do_prepare_twophase(self, connection, xid):
        connection.execute(
            sql.text("XA END :xid").bindparams(sql.bindparam("xid", xid, literal_execute=True))
        )
        connection.execute(
            sql.text("XA PREPARE :xid").bindparams(sql.bindparam("xid", xid, literal_execute=True))
        )
    ...
    def do_rollback_twophase(self, connection, xid, is_prepared=True, recover=False):
        ...
        connection.execute(
            sql.text("XA ROLLBACK :xid").bindparams(sql.bindparam("xid", xid, literal_execute=True))
        )
    ...
    def do_commit_twophase(self, connection, xid, is_prepared=True, recover=False):
        ...
        connection.execute(
            sql.text("XA COMMIT :xid").bindparams(sql.bindparam("xid", xid, literal_execute=True))
        )
