...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def _render_json_extract_from_binary(self, binary, operator, **kw):
        ...
        if binary.type._type_affinity is sqltypes.Integer:
            ...
            if binary.type.scale is not None and binary.type.precision is not None:
                ...
                type_expression = "ELSE CAST(JSON_EXTRACT(%s, %s) AS DECIMAL(%s, %s))" % (
                    self.process(binary.left, **kw),
                    self.process(binary.right, **kw),
                ...
                type_expression = "ELSE JSON_EXTRACT(%s, %s)+0.0000000000000000000000" % (
                    self.process(binary.left, **kw),
                    self.process(binary.right, **kw),
                )
    ...
    def visit_on_duplicate_key_update(self, on_duplicate, **kw):
        ...
        if on_duplicate._parameter_ordering:
            ...
            cols = [
                statement.table.c[key] for key in parameter_ordering if key in statement.table.c
                    ...
                    if isinstance(obj, elements.BindParameter) and obj.type._isnull:
                        ...
                        if requires_mysql8_alias:
                            column_literal_clause = (
                                f"{_on_dup_alias_name}." f"{self.preparer.quote(obj.name)}"
                            )
                        else:
        ...
        if requires_mysql8_alias:
            return f"AS {_on_dup_alias_name} " f"ON DUPLICATE KEY UPDATE {', '.join(clauses)}"
        else:
    ...
    def visit_join(self, join, asfrom=False, from_linter=None, **kwargs):
        ...
        return "".join(
            (
                ...
                join_type,
                self.process(join.right, asfrom=True, from_linter=from_linter, **kwargs),
                " ON ",
    ...
    def visit_empty_set_expr(self, element_types, **kw):
        return "SELECT %(outer)s FROM (SELECT %(inner)s) " "as _empty_set WHERE 1!=1" % {
            "inner": ", ".join("1 AS _in_%s" % idx for idx, type_ in enumerate(element_types)),
            "outer": ", ".join("_in_%s" % idx for idx, type_ in enumerate(element_types)),
        }
            ...
            if isinstance(length, dict):
                ...
                columns = ", ".join(
                    (
                        ...
                        else ("%s(%d)" % (expr, length[expr]) if expr in length else "%s" % expr)
                    )
...
class MySQLTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_VARCHAR(self, type_, **kw):
        if type_.length is not None:
            ...
            raise exc.CompileError("VARCHAR requires a length on dialect %s" % self.dialect.name)
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _parse_server_version(self, val):
        ...
        if server_version_info < (5, 0, 2):
            raise NotImplementedError(
                "the MySQL/MariaDB dialect supports server " "version info 5.0.2 and above."
            )
    ...
    def initialize(self, connection):
        ...
        self._needs_correct_for_88718_96365 = not self.is_mariadb and self.server_version_info >= (
            8,
        )
        ...
        self._requires_alias_for_on_duplicate_key = self._is_mysql and self.server_version_info >= (
            8,
            0,
            20,
        )
                        ...
                        sql.or_(
                            *(
                                sql.and_(
                                    ...
                                    sql.func.lower(_info_columns.c.column_name).in_(columns),
                                )
            ...
            try:
                ...
                if code == 1146:
                    ...
                    raise exc.UnreflectableTableError(
                        "Table or view named %s could not be " "reflected: %s" % (full_name, e)
                    ) from e
