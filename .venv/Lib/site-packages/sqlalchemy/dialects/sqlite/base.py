...
class SQLiteDialect(default.DefaultDialect):
        ...
        if self.dbapi is not None:
            if self.dbapi.sqlite_version_info < (3, 7, 16):
                util.warn(
                    ...
                    "no longer tries to rewrite these joins." % (self.dbapi.sqlite_version_info,)
                )
    ...
    def _sqlite_main_query(
        ...
        if not sqlite_include_internal:
            ...
            filter_table = ""
        query = f"SELECT name FROM {main} " f"WHERE type='{type_}'{filter_table} " "ORDER BY name"
        return query
    ...
    def _get_column_info(
        ...
        if generated:
            sqltext = ""
            if tablesql:
                pattern = r"[^,]*\s+GENERATED\s+ALWAYS\s+AS" r"\s+\((.*)\)\s*(?:virtual|stored)?"
