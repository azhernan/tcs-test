...
class OracleTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_INTERVAL(self, type_, **kw):
        return "INTERVAL DAY%s TO SECOND%s" % (
            ...
            type_.second_precision is not None and "(%d)" % type_.second_precision or "",
        )
...
class OracleCompiler(compiler.SQLCompiler):
    ...
    def _get_nonansi_join_whereclause(self, froms):
        ...
        def visit_join(join):
            if join.isouter:
                ...
                clauses.append(
                    visitors.cloned_traverse(join.onclause, {}, {"binary": visit_binary})
                )
            else:
            ...
            if (
                ...
                limitselect = sql.select(
                    *[
                        ...
                        if orig_select.selected_columns.corresponding_column(c) is not None
                    ]
                )
                ...
                if (
                    ...
                    limitselect = limitselect.prefix_with(
                        expression.text(
                            "/*+ FIRST_ROWS(%s) */" % self.process(limit_clause, **kwargs)
                        )
                    )
                ...
                if limit_clause is not None:
                    if select._simple_int_clause(limit_clause) and (
                        offset_clause is None or select._simple_int_clause(offset_clause)
                    ):
    ...
    def _visit_bitwise(self, binary, fn_name, custom_right=None, **kw):
        ...
        right = self.process(custom_right if custom_right is not None else binary.right, **kw)
...
class OracleExecutionContext(default.DefaultExecutionContext):
    def fire_sequence(self, seq, type_):
        return self._execute_scalar(
            "SELECT " + self.identifier_preparer.format_sequence(seq) + ".nextval FROM DUAL",
            type_,
        )
...
class OracleDialect(default.DefaultDialect):
    ...
    def has_sequence(self, connection, sequence_name, schema=None, dblink=None, **kw):
        ...
        query = select(dictionary.all_sequences.c.sequence_name).where(
            dictionary.all_sequences.c.sequence_name == self.denormalize_schema_name(sequence_name),
            dictionary.all_sequences.c.sequence_owner == self.denormalize_schema_name(schema),
        )
        ...
        if kind is ObjectKind.ANY:
            ...
            if ObjectKind.TABLE in kind:
                ...
                if has_mat_views and ObjectKind.MATERIALIZED_VIEW not in kind:
                    ...
                    query = query.where(
                        dictionary.all_objects.c.object_name.not_in(bindparam("mat_views"))
                    )
    ...
    def get_table_names(self, connection, schema=None, dblink=None, **kw):
        ...
        mat_query = select(dictionary.all_mviews.c.mview_name.label("table_name")).where(
            dictionary.all_mviews.c.owner == den_schema
        )

        query = (
            query.except_all(mat_query) if self._supports_except_all else query.except_(mat_query)
        )
    ...
    def get_temp_table_names(self, connection, dblink=None, **kw):
        ...
        if self.exclude_tablespaces:
            query = query.where(
                func.coalesce(dictionary.all_tables.c.tablespace_name, "no tablespace").not_in(
                    self.exclude_tablespaces
                )
            )
        query = query.where(
    ...
    def _index_query(self, owner):
        return (
            ...
            .join(
                ...
                sql.and_(
                    dictionary.all_ind_columns.c.index_name == dictionary.all_indexes.c.index_name,
                    dictionary.all_ind_columns.c.index_owner == dictionary.all_indexes.c.owner,
                ),
            )
            .outerjoin(
        ...
        return (
            ...
            .join(
                ...
                and_(
                    ...
                    dictionary.all_constraints.c.constraint_name == local.c.constraint_name,
                ),
            )
            .outerjoin(
                remote,
                and_(
                    ...
                    dictionary.all_constraints.c.r_constraint_name == remote.c.constraint_name,
                    or_(
