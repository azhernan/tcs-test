...
class PGCompiler(compiler.SQLCompiler):
    ...
    def visit_json_getitem_op_binary(self, binary, operator, _cast_applied=False, **kw):
        ...
        return self._generate_generic_binary(binary, " -> " if not _cast_applied else " ->> ", **kw)
    ...
    def visit_empty_set_expr(self, element_types, **kw):
        ...
        return "SELECT %s WHERE 1!=1" % (
            ", ".join(
                "CAST(NULL AS %s)"
                % self.dialect.type_compiler_instance.process(INTEGER() if type_._isnull else type_)
    ...
    def get_select_precolumns(self, select, **kw):
        ...
        if select._distinct or select._distinct_on:
            if select._distinct_on:
                return (
                    "DISTINCT ON ("
                    + ", ".join([self.process(col, **kw) for col in select._distinct_on])
                    + ") "
                )
            else:
                return "DISTINCT "
    ...
    def for_update_clause(self, select, **kw):
        ...
        if select._for_update_arg.of:
            ...
            tmp += " OF " + ", ".join(
                self.process(table, ashint=True, use_schema=False, **kw) for table in tables
            )
    ...
    def _on_conflict_target(self, clause, **kw):
        if clause.constraint_target is not None:
            ...
            target_text = "ON CONSTRAINT %s" % self.preparer.truncate_and_render_constraint_name(
...
class PGDDLCompiler(compiler.DDLCompiler):
    ...
    def visit_create_enum_type(self, create, **kw):
        ...
        return "CREATE TYPE %s AS ENUM (%s)" % (
            ...
            ", ".join(
                self.sql_compiler.process(sql.literal(e), literal_binds=True) for e in type_.enums
            ),
        )
    ...
    def visit_create_index(self, create, **kw):
        ...
        text += "(%s)" % (
            ", ".join(
                [
                    ...
                    + ((" " + ops[expr.key]) if hasattr(expr, "key") and expr.key in ops else "")
        ...
        if withclause:
            text += " WITH (%s)" % (
                ", ".join(
                    ["%s = %s" % storage_parameter for storage_parameter in withclause.items()]
                )
            )
    ...
    def _can_comment_on_constraint(self, ddl_instance):
        ...
        if constraint.name is None:
            raise exc.CompileError(
                f"Can't emit COMMENT ON for constraint {constraint!r}: " "it has no name"
            )
...
class PGTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_TIMESTAMP(self, type_, **kw):
        return "TIMESTAMP%s %s" % (
            ("(%d)" % type_.precision if getattr(type_, "precision", None) is not None else ""),
    ...
    def visit_ARRAY(self, type_, **kw):
        ...
        return re.sub(
            r"((?: COLLATE.*)?)$",
            (r"%s\1" % ("[]" * (type_.dimensions if type_.dimensions is not None else 1))),
            inner,
            count=1,
        )
...
class PGIdentifierPreparer(compiler.IdentifierPreparer):
    ...
    def format_type(self, type_, use_schema=True):
        if not type_.name:
            raise exc.CompileError(f"PostgreSQL {type_.__class__.__name__} type requires a name.")
...
class PGDialect(default.DefaultDialect):
    ...
    def _split_multihost_from_url(self, url: URL) -> Union[
        ...
        if "host" in url.query:
            if isinstance(url.query["host"], (list, tuple)):
                ...
                if "port" not in url.query and len(hosts) == 1 and ":" in hosts[0]:
                    ...
                    host_port_match = re.match(r"^([a-zA-Z0-9\-\.]*)(?:\:(\d*))?$", hosts[0])
                    if host_port_match:
                        integrated_multihost = True
        ...
        if ports and (
            ...
            or (hosts and ports and len(hosts) != len(ports) and (len(hosts) > 1 or len(ports) > 1))
        ):
    ...
    def get_multi_columns(self, connection, schema, filter_names, scope, kind, **kw):
        ...
        domains = {
            ...
            for d in self._load_domains(connection, schema="*", info_cache=kw.get("info_cache"))
        }
        ...
        enums = dict(
            ...
            for rec in self._load_enums(connection, schema="*", info_cache=kw.get("info_cache"))
        )
    ...
    def _constraint_query(self, is_unique):
        con_sq = (
            select(
                ...
                sql.func.generate_subscripts(pg_catalog.pg_constraint.c.conkey, 1).label("ord"),
            ...
            select(
                ...
                sql.case(
                    (
                        ...
                        pg_catalog.pg_get_constraintdef(pg_catalog.pg_constraint.c.oid, True),
                    ),
                    else_=None,
                ),
    ...
    def _index_query(self):
        ...
        idx_sq = (
            select(
                ...
                sql.func.generate_subscripts(pg_catalog.pg_index.c.indkey, 1).label("ord"),
            )
            .where(
        ...
        attr_sq = (
            select(
                ...
                sql.case(
                    (
                        idx_sq.c.attnum == 0,
                        pg_catalog.pg_get_indexdef(idx_sq.c.indexrelid, idx_sq.c.ord + 1, True),
                    ),
        ...
        cols_sq = (
            select(
                ...
                sql.func.array_agg(aggregate_order_by(attr_sq.c.element, attr_sq.c.ord)).label(
                    "elements"
                ),
                sql.func.array_agg(aggregate_order_by(attr_sq.c.is_expr, attr_sq.c.ord)).label(
                    "elements_is_expr"
                ),
            )
        ...
        return (
            ...
            .outerjoin(
                pg_catalog.pg_constraint,
                sql.and_(
                    pg_catalog.pg_index.c.indrelid == pg_catalog.pg_constraint.c.conrelid,
                    pg_catalog.pg_index.c.indexrelid == pg_catalog.pg_constraint.c.conindid,
    ...
    def _enum_query(self, schema):
        ...
        query = (
            select(
                ...
                pg_catalog.pg_type_is_visible(pg_catalog.pg_type.c.oid).label("visible"),
