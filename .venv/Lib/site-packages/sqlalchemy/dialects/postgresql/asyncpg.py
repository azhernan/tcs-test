        ...
        async with adapt_connection._execute_mutex:
            ...
            try:
                ...
                if self.server_side:
                    ...
                    reg = re.match(r"(?:SELECT|UPDATE|DELETE|INSERT \d+) (\d+)", status)
                    if reg:
        ...
        if multihosts:
            ...
            if len(multihosts) == 1:
                ...
                raise exc.ArgumentError(
                    "All hosts are required to be present" " for asyncpg multiple host URL"
                )
            elif not all(multiports):
                raise exc.ArgumentError(
                    "All ports are required to be present" " for asyncpg multiple host URL"
                )
            else:
