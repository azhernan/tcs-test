...
class CursorResultMetaData(ResultMetaData):
    ...
    def _remove_processors(self) -> CursorResultMetaData:
        ...
        return self._make_new_metadata(
            ...
            keymap={key: value[0:5] + (None,) + value[6:] for key, value in self._keymap.items()},
            keys=self._keys,
        ...
        keymap.update(
            {
                key: (
                    ...
                    (value[0] + offset if value[0] is not None and key not in keymap else None),
                    value[1] + offset,
                    *value[2:],
                )
    ...
    def __init__(
        ...
        if context.result_column_struct:
            ...
            result_columns = cols_are_ordered = num_ctx_cols = ad_hoc_textual = (  # type: ignore
                loose_column_name_matching
        ...
        if not num_ctx_cols and context._translate_colname:
            self._keymap.update(
                {
                    metadata_entry[MD_UNTRANSLATED]: self._keymap[metadata_entry[MD_LOOKUP_KEY]]
                    for metadata_entry in raw
    ...
    def _raise_for_ambiguous_column_name(self, rec):
        raise exc.InvalidRequestError(
            "Ambiguous column name '%s' in " "result set column descriptions" % rec[MD_LOOKUP_KEY]
        )
...
class CursorResult(Result[_T]):
    ...
    def _init_metadata(self, context, cursor_description):
        if context.compiled:
            ...
            if (
                not context.execution_options.get("_result_disable_adapt_to_context", False)
    ...
    def merge(self, *others: Result[Any]) -> MergedResult[Any]:
        ...
        if self.context._has_rowcount:
            merged_result.rowcount = sum(
                cast("CursorResult[Any]", result).rowcount for result in (self,) + others
            )
