    ...
    * :attr:`_engine.URL.query`: an immutable mapping representing the query
    ...
    def difference_update_query(self, names: Iterable[str]) -> URL:
        ...
        return URL(
            ...
            self.port,
            self.database,
            util.immutabledict({key: self.query[key] for key in set(self.query).difference(names)}),
        )
...
def _parse_url(name: str) -> URL:
    ...
    if m is not None:
        ...
        raise exc.ArgumentError("Could not parse SQLAlchemy URL from string '%s'" % name)
