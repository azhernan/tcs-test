...
class Connection(ConnectionEventsTarget, inspection.Inspectable["Inspector"]):
    ...
    def _schema_translate_map(self) -> Optional[SchemaTranslateMapType]:
        schema_translate_map: Optional[SchemaTranslateMapType] = self._execution_options.get(
    ...
    def begin_twophase(self, xid: Optional[Any] = None) -> TwoPhaseTransaction:
        ...
        if self._transaction is not None:
            raise exc.InvalidRequestError(
                "Cannot start a two phase transaction when a transaction " "is already in progress."
            )
        if xid is None:
    ...
    def in_nested_transaction(self) -> bool:
        """Return True if a transaction is in progress."""
        return self._nested_transaction is not None and self._nested_transaction.is_active

    def _is_autocommit_isolation(self) -> bool:
        opt_iso = self._execution_options.get("isolation_level", None)
        return bool(
            opt_iso == "AUTOCOMMIT"
            or (opt_iso is None and self.engine.dialect._on_connect_isolation_level == "AUTOCOMMIT")
        )
    ...
    def _begin_impl(self, transaction: RootTransaction) -> None:
        if self._echo:
            if self._is_autocommit_isolation():
                self._log_info("BEGIN (implicit; DBAPI should not BEGIN due to " "autocommit mode)")
            else:
    ...
    def scalars(
        ...
        *,
        ...
        return self.execute(statement, parameters, execution_options=execution_options).scalars()
    ...
    def _execute_function(
        ...
        return self._execute_clauseelement(func.select(), distilled_parameters, execution_options)
    ...
    def _execute_default(
        ...
        if self._has_events or self.engine._has_events:
            (
                ...
                event_multiparams,
                event_params,
            ) = self._invoke_before_exec_event(default, distilled_parameters, execution_options)
        else:
    ...
    def _execute_clauseelement(
        ...
        if has_events:
            (
                ...
                event_multiparams,
                event_params,
            ) = self._invoke_before_exec_event(elem, distilled_parameters, execution_options)
    ...
    def _execute_compiled(
        ...
        if self._has_events or self.engine._has_events:
            (
                ...
                event_multiparams,
                event_params,
            ) = self._invoke_before_exec_event(compiled, distilled_parameters, execution_options)
    ...
    def _handle_dbapi_exception_noconnection(
        ...
        if is_disconnect is None:
            is_disconnect = isinstance(e, dialect.loaded_dbapi.Error) and dialect.is_disconnect(
                e, None, None
            )
        ...
        if should_wrap:
            sqlalchemy_exception = exc.DBAPIError.instance(
                ...
                hide_parameters=(engine.hide_parameters if engine is not None else False),
...
class Engine(ConnectionEventsTarget, log.Identified, inspection.Inspectable["Inspector"]):
    ...
    def __init__(
        ...
        if query_cache_size != 0:
            self._compiled_cache = util.LRUCache(query_cache_size, size_alert=self._lru_size_alert)
        else:
