...
class DefaultDialect(Dialect):
    ...
    def initialize(self, connection):
        ...
        if self.label_length and self.label_length > self.max_identifier_length:
            raise exc.ArgumentError(
                ...
                " maximum identifier length of %d" % (self.label_length, self.max_identifier_length)
            )
    ...
    def _set_connection_characteristics(self, connection, characteristics):
        ...
        if connection.in_transaction():
            trans_objs = [
                (name, obj) for name, obj, _ in characteristic_values if obj.transactional
            ]
            if trans_objs:
                raise exc.InvalidRequestError(
        ...
        for _, characteristic, value in characteristic_values:
            characteristic.set_connection_characteristic(self, connection, dbapi_connection, value)
                ...
                if imv.num_sentinel_columns and not imv_batch.is_downgraded:
                    ...
                    if not _sentinel_proc_initialized:
                        if composite_sentinel:
                            ...
                            ).type._cached_result_processor(self, cursor.description[-1][1])
                    ...
                    if composite_sentinel:
                        rows_by_sentinel = {
                            tuple(
                                ...
                                for val, proc in zip(row[-_nsc:], _composite_sentinel_proc)
                            ): row
                            for row in rows
                        }
...
class DefaultExecutionContext(ExecutionContext):
    ...
    def _setup_out_parameters(self, result):
        ...
        out_bindparams = [
            (param, name) for param, name in compiled.bind_names.items() if param.isoutparam
        ]
        out_parameters = {}
    ...
    def _setup_dml_or_text_result(self):
        ...
        if self.isinsert:
            if self._is_implicit_returning:
                ...
                self.inserted_primary_key_rows = self._setup_ins_pk_from_implicit_returning(
                    result, rows
                )
    ...
    def _prepare_set_input_sizes(
        ...
        for key, bindparam in items:
            ...
            if key in self._expanded_parameters:
                ...
                escaped_name = (
                    escaped_bind_names.get(key, key) if escaped_bind_names is not None else key
                )
